...
| 6 | 境界のある領域での temporal blocking と MPI 分割 | 中 | **fork にコミット済**(tb-boundaries 54838f6, 9c66835, 5cd015c, 5c162c8)・下記に実装記録 |
...
- 通信と計算の重ね合わせ(5c162c8): ブロックは上端の角から下へ順に回り、各ブロックは
  各サブステップで上隣のブロックの帯を借りるので順序は変えられない。代わりに待ちを分け、
  下側 halo に触れない「上側の箱」(各軸で添字 `[0, m-1-d)`、`d` は下側 halo の幅を
  ブロック数で数えたもの)のブロックを、横と上からの帯を置いた直後に回し、その間に
  下からの帯を受信し、残りのブロックはその完了後に回す(全周期版の分割を軸ごとの halo 幅に
  一般化したもの、`waitAndCopyHaloOn`)。全周期・非分割の生成 C は不変。同一機の共有メモリ上では
  交換が速く実測時間は変わらない(2 ランク 10.0 対 10.1 ms/ステップ、8 ランク 8.15 対 8.36)。
