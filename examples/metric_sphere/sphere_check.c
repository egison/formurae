...
#include <stdlib.h>
#include <math.h>
#include "metric_sphere.h"

/* Heat on a band of the unit sphere, colatitude 1+theta, Neumann
 * (mirror) walls in theta, periodic in phi.  Checks: agreement with an
 * independent reference implementation, exact conservation of the
 * metric-weighted heat sum(sin(1+theta) u) (zero wall flux + flux
 * form), and the max principle via the generated reductions. */
...
static void refRun(int steps, double dt, double hx, double hy) {
  for (int i = 0; i < NX; i++)
    for (int j = 0; j < NY; j++) {
      double th = i * hx, ph = j * hy;
      ru[i][j] = exp(-4.0 * (th - 0.5) * (th - 0.5)) * exp(cos(ph) - 1.0);
    }
  for (int t = 0; t < steps; t++) {
    /* theta fluxes at i+1/2 (0 at both walls: mirror ghosts) */
    for (int j = 0; j < NY; j++) {
      f1[0][j] = 0.0;
      f1[NX][j] = 0.0;
    }
    for (int i = 0; i + 1 < NX; i++)
      for (int j = 0; j < NY; j++)
        f1[i + 1][j] = sin(1.0 + (i + 0.5) * hx) * (ru[i + 1][j] - ru[i][j]) / hx;
    for (int i = 0; i < NX; i++) {
      double B = 1.0 / sin(1.0 + i * hx), sg = sin(1.0 + i * hx);
      for (int j = 0; j < NY; j++) {
...
int main(int argc, char **argv) {
  ...
  printf("sphere band: conservation + reference + max principle: [%s]\n", ok ? "OK" : "NG");
  Formura_Finalize();
  return ok ? 0 : 1;
}
