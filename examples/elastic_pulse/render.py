...
points to physical positions, colors the two wave indicators, and writes the
figure, the plot data, and a record of the runs.  Nothing here feeds back
into a simulation.
"""
import argparse
import csv
...
TEXT = {
    "en": {"rows": ["Cartesian coordinates", "Non-orthogonal coordinates"],
           "columns": ["$t = 0.4$", "$t = 0.8$", "$t = 0.8$, anisotropic material"],
           "p": "P wave: $a\\,|\\nabla\\cdot v|/V_0$", "s": "S wave: $a\\,|\\nabla\\times v|/V_0$",
           "legend": "dashed circles: radii $c_P t$ and $c_S t$; black dot: source; thin lines: every 16th grid line"},
    "ja": {"rows": ["直交座標", "非直交座標"],
           "columns": ["$t = 0.4$", "$t = 0.8$", "$t = 0.8$，異方性材料"],
           "p": "P 波：$a\\,|\\nabla\\cdot v|/V_0$", "s": "S 波：$a\\,|\\nabla\\times v|/V_0$",
           "legend": "破線円：半径 $c_P t$ と $c_S t$；黒点：波源；細線：16 本ごとの格子線"},
}
...
def load_slice(directory, step):
    ...
    for path in sorted((directory / "data").glob(f"slice-{step:07d}-rank-*.bin")):
        with path.open("rb") as file:
            nx, ny, nz, stamp, full = np.fromfile(file, dtype="=i4", count=5)
...
def physical(meta, n):
    # node (i, j) of the chart maps to (x + shear sin(wavenumber y), y); the
    # source at (x0, y0) is moved to the origin for display
    p = meta["parameters"]
    shear, wavenumber = float(p["shear"]), float(p["wavenumber"])
    dx = LENGTH / n
    i, j = np.meshgrid(np.arange(n), np.arange(n), indexing="ij")
    x, y = i * dx, j * dx
    return x + shear * np.sin(wavenumber * y) - float(p["x0"]), y - float(p["y0"])
...
def panel(ax, directory, step, radius, circles, grid_every=16):
    ...
    n = fields["compression"].shape[0]
    x, y = physical(meta, n)
    p = radius * np.abs(fields["compression"])
    s = radius * fields["shearing"]
    # periodic wrap: append the first row/column so that the mesh closes
    def close(a):
        a = np.concatenate([a, a[:1, :]], axis=0)
        return np.concatenate([a, a[:, :1]], axis=1)
    dx = LENGTH / n
    xc, yc = close(x), close(y)
    xc[-1, :] += LENGTH
    yc[:, -1] += LENGTH
    colors = rgb(close(p), close(s))
    mesh = ax.pcolormesh(xc - dx / 2, yc - dx / 2, np.zeros((n, n)), shading="flat",
                         edgecolors="none", rasterized=True)
    mesh.set_array(None)
    mesh.set_facecolor(colors[:-1, :-1].reshape(-1, 3))
    for start in range(0, n, grid_every):
        ax.plot(x[start, :], y[start, :], color="0.55", lw=0.35)
        ax.plot(x[:, start], y[:, start], color="0.55", lw=0.35)
    for r in circles:
        ax.add_patch(Circle((0, 0), r, fill=False, color="black", lw=0.7, ls=(0, (4, 3))))
    ax.plot([0], [0], "k.", ms=4)
    ax.set(xlim=(-LENGTH / 2, LENGTH / 2), ylim=(-LENGTH / 2, LENGTH / 2), aspect="equal",
           xticks=[], yticks=[])
    return p.max(), s.max()
...
def figure(runs, lang, output):
    ...
    meta = metadata(runs["cartesian"])
    # dt = 0.1 dx and L = 4: the snapshots t = 0.4 and 0.8 are steps N and 2N
    steps = (meta["grid"][0], 2 * meta["grid"][0])
    radius = float(meta["parameters"]["radius"])
    dt = float(meta["parameters"]["dt"].split("*")[0]) * LENGTH / meta["grid"][0]
    speeds = (2.0, 1.0)      # lambda = 2, mu = rho0 = 1
    fig, axes = plt.subplots(2, 3, figsize=(7.0, 5.15))
    maxima = {}
    for row, case in enumerate(("cartesian", "sheared")):
        ...
        axes[row, 0].set_ylabel(text["rows"][row], fontsize=9, fontproperties=font)
...
def main():
    ...
    parser.add_argument("--runs", type=Path, default=ROOT / ".build/elastic_pulse/demo")
    parser.add_argument("--output", type=Path, default=HERE / "results")
    args = parser.parse_args()
    runs = {name: args.runs / name for name in ("cartesian", "sheared", "cartesian-anisotropic", "sheared-anisotropic")}
    ...
    # agreement of the two coordinate systems on the same physical box
    tables = {name: radii(runs[name]) for name in ("cartesian", "sheared")}
    record["chart_agreement"] = {
        key: max(abs(a[column] - b[column]) for a, b in zip(tables["cartesian"], tables["sheared"]) if 0.3 <= a[0] <= 0.7)
    ...
    record["chart_agreement"]["energy_relative"] = max(
        abs(a[5] - b[5]) / a[5] for a, b in zip(tables["cartesian"], tables["sheared"]))
    record["grid_spacing"] = LENGTH / metadata(runs["cartesian"])["grid"][0]
    for lang in ("en", "ja"):
        output = args.output / f"elastic-pulse-{lang}.png"
