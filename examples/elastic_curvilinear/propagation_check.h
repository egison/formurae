/* Localized, three-dimensional pulse evolved by the generated kernel.
...
static void write_slice(const char *directory,const double *state,int step) {
  char path[4096];
  int length=snprintf(path,sizeof(path),"%s/slice-%04d.dat",directory,step);
  if(length<0 || (size_t)length>=sizeof(path)) exit(2);
  FILE *out=fopen(path,"w");if(!out) {perror(path);exit(2);}
  fprintf(out,"radial transverse x z speed vx vy vz\n");
  int ntrans=dims[SPHERICAL ? 1 : 2];
  for(int i=0;i<dims[0];i++) for(int a=0;a<ntrans;a++) {
    int q[3]={i,SPHERICAL ? a : 0,SPHERICAL ? 0 : a};
    int p=offset(q[0],q[1],q[2]);
    double X[3],basis[3][3],scale[3],v[3]={0,0,0};
    cartesian_geometry(q,X,basis,scale);
    for(int c=0;c<3;c++) for(int k=0;k<3;k++)
      v[k]+=state[c*count+p]*scale[c]*basis[c][k];
    fprintf(out,"%d %d %.17g %.17g %.17g %.17g %.17g %.17g\n",i,a,
      X[0],X[2],physical_speed(state,p,scale),v[0],v[1],v[2]);
  }
  if(fclose(out)) {perror(path);exit(2);}
}
...
int main(int argc,char **argv) {
  ...
  for(int step=1;step<=frames[3];step++) {
    ...
    if(check || step%16==0) {
      ...
      if(step==frames[1] || step==frames[2] || step==frames[3]) {
        write_slice(directory,state,step);
