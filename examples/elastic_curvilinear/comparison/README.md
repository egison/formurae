...
2026-09-10に円筒座標の波動モデルを削除したため，現在のスクリプトは
その全体モデルを使う2入力を除く13入力と，2026-09-12に追加した三次流束の
`flux_cubic`（下記）の合計14入力を対象とする．円筒座標の計量を使う
独立した構成則の検査は保持する．以下の15入力の表と保存済みJSONは削除前の実測記録である．
...
保存性の違いは三次の流束で確かめる（`fixtures/flux_cubic.fme`）．
$\partial_x(u^3/3)$ の流束形と積の微分則で書き換えた $u^2\partial_xu$ の
両方を生成させ，生成された更新式を8点の周期格子 $u=(1,4,9,2,7,3,5,8)$，
$h=0.25$ で総和する．前者は隣り合う項が打ち消し合って $2.8\times10^{-14}$
（丸め誤差），後者は $-292$ になる（`results.json` の `periodic_sums`）．
