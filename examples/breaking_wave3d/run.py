...
def build(directory, grid=(128, 64, 16), overrides=None):
    directory = Path(directory).resolve()
    directory.mkdir(parents=True, exist_ok=True)
    source = (HERE / (NAME + ".fme")).read_text()
    for name, value in {"nx": grid[0], "ny": grid[1], "nz": grid[2], **(overrides or {})}.items():
    ...
    config = ["length_per_node: " + json.dumps(list(grid)),
              "grid_per_node: " + json.dumps(list(grid)), "mpi_shape: [1, 1, 1]",
              ...
              "span_velocity = absmax spanVelocity, span_difference = absmax spanDifference]"]
...
def main():
    ...
    parser.add_argument("--grid", type=int, nargs=3, default=[128, 64, 16])
    parser.add_argument("--steps", type=int, default=760)
    parser.add_argument("--every", type=int, default=20)
    parser.add_argument("--param", action="append", default=[])
    parser.add_argument("--output", type=Path, default=ROOT / ".build/breaking_wave3d/demo")
