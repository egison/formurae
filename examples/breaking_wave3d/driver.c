...
static void dump(Formura_Navi n, const char *directory) {
  ...
  int32_t header[] = {n.total_grid_x, n.total_grid_y, n.total_grid_z, n.time_step/5};
  ...
  for (int i=n.lower_x; i<n.upper_x; ++i)
    for (int j=n.lower_y; j<n.upper_y; ++j)
      ...
      int32_t index[] = {((i+n.offset_x)%n.total_grid_x+n.total_grid_x)%n.total_grid_x,
                         ...
                         ((k+n.offset_z)%n.total_grid_z+n.total_grid_z)%n.total_grid_z};
      /* Compact output only; the generated solver stores doubles. */
      float values[] = {(float)formura_data.fraction[i][j][k],
                        (float)formura_data.wall[i][j][k]};
...
static int report(Formura_Navi n) {
  printf("%d,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g\n", n.time_step/5,
         ...
         n.reduce_span_velocity, n.reduce_span_difference);
  fflush(stdout);
...
int main(int argc, char **argv) {
  ...
  puts("step,water,bank,speed,rmin,rmax,overhang,bad,span_velocity,span_difference");
  int ok=report(n);
  dump(n, argv[3]);
