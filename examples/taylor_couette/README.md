...
間の非圧縮流れを，子午面（円筒の軸を含む縦断面）$(r,z)$ の軸対称モデルとして計算する．$z$ は周期的である．
...
保存済みの軸対称な速度場を円筒座標から立体に描き直し，見えている各面に，その位置の速度のうち
面に沿う成分を白い矢印で示す．立体図の矢印の位置は固定する．矢印の長さ・幅・先端の形は，
立体図・中央図・右下の流線・別の静止図を通じて，画像上で同じ大きさに揃える．速さは矢印の大きさに反映しない．
静止図も動画と同じ横幅 1920 ピクセルで出力し，gallery で同じ幅に縮小されたときにも矢印の大きさが揃うようにする．
面に垂直な成分は含めず，次のように描き分ける．

- 内側の流体の円筒面（$r=1.10$）：その位置の周方向・軸方向の成分 $(u_\theta,u_z)$．
  色はその位置の軸方向速度 $u_z$ で，赤は上昇，青は下降を示す．内筒の壁面 $r=1$ と区別し，
  色も矢印も実際の $r=1.10$ の位置に描く．
- 外側の流体の円筒面（$r=1.90$）：その位置の $(u_\theta,u_z)$．色は同じく $u_z$ で，
  内側の円筒面と共通の色尺度（左端）を使う．内外で同じ色は同じ軸方向速度を意味する．
- 二つの長方形の切り口（$1.10\le r\le1.90$）：半径方向・軸方向の成分 $(u_r,u_z)$．
  色はこの縦断面内の速さで，右下の縦断面と同じ尺度を使う．
- 上の扇形（$z=L_z$）：半径方向・周方向の成分 $(u_r,u_\theta)$．色は周方向速度で，中央図と同じ尺度を使う．
  周期的な計算領域を切った流体の断面であり，物理的な蓋ではない．表示用の補間も $z=0$ と $z=L_z$ の周期性を保つ．

内筒（$r=1$）は上部の灰色の断面と回転する印で示す．その周りの細い帯も流体の水平断面である．
外壁（$r=2$）は静止した灰色の破線で示す．流れを描く二つの円筒面は，各壁から隙間幅の 10% だけ
流体側に入った $r=1.10$ と $r=1.90$ の実際の位置に置き，縁を橙色にする．中央の軸方向の図にも
同じ二つの半径の橙色の円を描き，流体の断面と壁の位置を対応させる．壁面で流体の速度を壁の速度に
合わせる条件により，内筒壁面の周方向速度は $\Omega_1 R_1=1$，軸方向速度は 0，外壁の速度は 0 である．
立体図では速度の向きを画面上の向きに変換してから矢印を描き，斜めから見た面でも矢印が小さくならないようにする．
矢印全体がその面に収まることを確認し，狭い部分では矢印を縮めずに数を減らす．中央図の矢印にも同じ形を使い，
半径によって長さが変わる線は付けない．流線上の矢印も同じ描画処理で表示する．
二つの円筒面を含む共通の色尺度を，すべてのフレームで固定する．
...
右上は周方向速度から，渦のない基準のクエット流れの速度 $Ar+B/r$ を引いた値である．
赤は基準より速い，白は同じ，青は基準より遅いことを示し，青は逆回転を意味しない．
右下は縦断面内の速さ $\sqrt{u_r^2+u_z^2}$ と流線で，紫（0 に近い）から緑，黄へと速くなる．
この速さには周方向の成分を含めない．各図の下に色の意味を記し，別の静止図にも同じ注釈を付ける．
破線が中央図の断面位置を示す．

500 ステップ（時間 1.5）ごとに保存された速度を描画用に線形補間し，計算時間の 6 倍速，
30 fps，1920×960 の約 30 秒の動画にする．中央図の矢印の角度は補間した角速度に従って動かす．
内筒の回転が逆向きに見えないよう，1 コマあたりの回転角が十分小さいことを確認する．
右側の色尺度は最終状態に固定し，流線の形は最も近い保存時刻の速度で描く．
立体図の各面の色と矢印は，記録間を補間した速度で毎フレーム更新する．
