...
表示範囲であり，蓋は置かない．切り口の色尺度は右下の縦断面と共通である．

外壁（$r=2$）は静止した灰色の枠線で示す．流れを描く円筒断面は，外壁から隙間幅の 10% だけ
内側にある実際の $r=1.90$ の位置に置き，縁を橙色にする．中央の軸方向の図にも同じ半径の
橙色の円を描き，この流体の断面と壁の位置を対応させる．外壁そのものでは，静止した壁に
流体の速度を合わせる条件により速度は 0 である．
円筒断面の色はその位置の軸方向速度 $u_z$ で，赤は上昇，青は下降を示し，左端の色尺度を使う．
白い矢印も同じ半径に置き，断面に沿う速度成分 $(u_\theta,u_z)$ を示す．矢印の位置は固定し，向きと長さを保存した速度から
描くので，周方向の回転と上下方向の流れを一緒に読める．半径方向の成分はこの矢印に含めない．
色尺度と矢印の長さの尺度は，すべてのフレームで固定する．
...
流体の円筒断面の色と矢印は，記録間を補間した速度で毎フレーム更新する．
