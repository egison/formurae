...
- 内筒面（$r=1$）：周方向・軸方向の成分 $(u_\theta,u_z)$．壁面で流体の速度を壁の速度に合わせる条件により，
  周方向速度は $\Omega_1 R_1=1$，軸方向速度は 0 となる．縦の印は内筒とともに回転する．
- 外側の流体の円筒面（$r=1.90$）：その位置の $(u_\theta,u_z)$．色は軸方向速度 $u_z$ で，
  赤は上昇，青は下降を示し，左端の色尺度を使う．
- 二つの長方形の切り口（$1\le r\le1.90$）：半径方向・軸方向の成分 $(u_r,u_z)$．
...
外壁（$r=2$）は静止した灰色の破線で示す．流れを描く円筒面は，外壁から隙間幅の 10% だけ
内側にある実際の $r=1.90$ の位置に置き，縁を橙色にする．中央の軸方向の図にも同じ半径の
橙色の円を描き，この流体の断面と壁の位置を対応させる．外壁そのものの速度は 0 である．
