...
外側の曲面には，外壁から隙間幅の 10% だけ内側にある $r=1.90$ の流れを投影する．
外壁そのもの（$r=2$）では，静止した壁に流体の速度を合わせる条件により速度は 0 である．
曲面の色は軸方向速度 $u_z$ で，赤は上昇，青は下降を示し，左端の色尺度を使う．白い矢印は
曲面に沿う速度成分 $(u_\theta,u_z)$ を示す．矢印の位置は固定し，向きと長さを保存した速度から
...
外側の曲面の色と矢印は，記録間を補間した速度で毎フレーム更新する．
