...
The shell is the region between the spheres R = 1 and R = 2, between the
cones of colatitude arctan 2 and pi - arctan 2, and around the full circle
of longitude: the grid holds nr nodes along the radius (spacing 1/(nr-1),
walls on both ends), nt nodes along the colatitude (spacing
(pi - 2 arctan 2)/(nt-1), walls on both ends), and np cells along the
periodic longitude (spacing 2 pi / np).
"""
import argparse
import hashlib
import json
...
def build(directory, case="spherical", grid=(65, 65, 128), mpi=(1, 1, 1), blocking=4,
    ...
    # Whole-line substitutions replace the initial state (the Bessel mode
    # instead of the pulse); extra_fme appends declarations, initializers and
    # updates.
    ...
    if fresh:
        ...
        cache = ROOT / ".build/elastic_shell/normalized" / key
...
def run(directory, steps, every, mpi=(1, 1, 1), dump=True, full=False):
    ...
    if (os.environ.get("ELASTIC_SHELL_REUSE") and metadata.get("steps") == steps
...
def main():
    ...
    parser.add_argument("--source", default=NAME + ".fme",
                        help="model file in this directory (the anisotropic variant is elastic_shell_anisotropic.fme)")
    parser.add_argument("--grid", type=int, nargs=3, default=[97, 65, 256],
                        metavar=("NR", "NT", "NP"), help="radial nodes, colatitude nodes, longitude cells")
    ...
    parser.add_argument("--steps", type=int, default=0, help="default: 16 (nr - 1), which is t = 0.8 with dt = 0.05 dr")
    parser.add_argument("--every", type=int, default=0, help="default: nr - 1, which is every 0.05 in time")
    parser.add_argument("--param", action="append", default=[], metavar="NAME=VALUE")
    parser.add_argument("--output", type=Path, default=ROOT / ".build/elastic_shell/demo")
    ...
    steps = args.steps or 16 * (args.grid[0] - 1)
    every = args.every or args.grid[0] - 1
    if min(*args.grid, *args.mpi, steps, every) < 1 or args.blocking < 0:
        parser.error("sizes and intervals must be positive")
    if steps % every or every % (args.blocking or 1):
    ...
    for case in CASES if args.case == "all" else [args.case]:
        directory = build(args.output / (case + label), case, tuple(args.grid), tuple(args.mpi),
                          args.blocking, overrides, source=args.source, fresh=args.fresh)
        run(directory, steps, every, tuple(args.mpi))
