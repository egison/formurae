...
def load():
    ...
    assert len(basic)==1 and basic[0]['passed']
    ...
    return report,beach,basic[0]['measurements'],review


def card(lang,report,beach,basic,review):
    ...
    assert period_cases
    period_text=', '.join(f"{c['record']['grid'][0]}×{c['record']['grid'][1]}: {100*e:.2f}%" for c,e in zip(period_cases,period_errors))
    ...
    if lang=='ja':
        ...
        metrics=[('水量の最大相対変化',f'{basic["relative_water_mass_drift"]:.3e}'),
        ...
        metrics=[('Maximum relative water-mass drift',f'{basic["relative_water_mass_drift"]:.3e}'),
...
def main():
    report,beach,basic,review=load()
    for lang in ['ja','en']:
        path=run.ROOT/'html'/lang/'waves.html'; text=path.read_text()
        article,title=card(lang,report,beach,basic,review)
