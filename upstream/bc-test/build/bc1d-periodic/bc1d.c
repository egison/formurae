...
send_buf1_p1[i1].q = formura_data.q[i1+62];
}

MPI_Request send_req_p1;
MPI_Isend(send_buf1_p1,sizeof(send_buf1_p1),MPI_BYTE,n->rank_p1,0,n->mpi_world,&send_req_p1);
MPI_Request recv_req_m1;
MPI_Irecv(recv_buf1_m1,sizeof(recv_buf1_m1),MPI_BYTE,n->rank_m1,0,n->mpi_world,&recv_req_m1);
MPI_Wait(&send_req_p1,MPI_STATUS_IGNORE);
MPI_Wait(&recv_req_m1,MPI_STATUS_IGNORE);
for(int i1 = 0; i1 < 2; i1 += 1) {
buff.q[i1] = recv_buf1_m1[i1].q;
}

for(int i1 = 0; i1 < 64; i1 += 1) {
buff.q[i1+2] = formura_data.q[i1];
}
