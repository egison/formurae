...
tmp_floor.r[i1+16] = formura_data.r[i1];
}

for(int i1 = 0; i1 < 16; i1 += 1) {
send_buf8_p1[i1].q = formura_data.q[i1+48];
send_buf8_p1[i1].r = formura_data.r[i1+48];
}
...
rslt.r[i1] = tmp_floor.r[i1+72-8*j1];
}
...
tmp_floor.r[i1+72-8*j1] = rslt.r[i1];
}
...
tmp_floor.r[i1] = recv_buf8_m1[i1].r;
}
...
formura_data.r[i1] = tmp_floor.r[i1];
}
