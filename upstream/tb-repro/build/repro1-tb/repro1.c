...
static Formura_Comm_Buff send_buf4_p1[8];
static Formura_Comm_Buff recv_buf4_m1[8];
...
for(int i1 = 0; i1 < 64; i1 += 1) {
tmp_floor.q[i1+8] = formura_data.q[i1];
}

for(int i1 = 0; i1 < 8; i1 += 1) {
send_buf4_p1[i1].q = formura_data.q[i1+56];
}

MPI_Request send_req_p1;
MPI_Isend(send_buf4_p1,sizeof(send_buf4_p1),MPI_BYTE,n->rank_p1,0,n->mpi_world,&send_req_p1);
MPI_Request recv_req_m1;
MPI_Irecv(recv_buf4_m1,sizeof(recv_buf4_m1),MPI_BYTE,n->rank_m1,0,n->mpi_world,&recv_req_m1);
for(int j1 = 0; j1 < 7; j1 += 1) {
for(int i1 = 0; i1 < 8; i1 += 1) {
rslt.q[i1] = tmp_floor.q[i1+64-8*j1];
}
...
Formura_Step(&buff,&rslt,*n,+64-8*j1);
for(int i1 = 0; i1 < 2; i1 += 1) {
...
for(int i1 = 0; i1 < 8; i1 += 1) {
tmp_floor.q[i1+64-8*j1] = rslt.q[i1];
}
...
for(int i1 = 0; i1 < 8; i1 += 1) {
tmp_floor.q[i1] = recv_buf4_m1[i1].q;
}
