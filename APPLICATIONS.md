...
| 12 | Kuramoto–Sivashinsky(時空カオス) | ∂t u = −∂x(u²/2) − ∇²u − ∇⁴u | 保存項は `gridD_x`、4 階項は中間場への2階差分×2段 | Lyapunov 内で参照と 3.6e-14・アトラクタ統計・Σu 5e-12 | **済** (examples/ks3d) |
...
μ を `local` として書くと、`post-fec` が .fmr の中間格子場としてmaterializeし、
...
`GeometryNF` として FEIR へ出す。`lb u` 自体は保存流束の離散 request として残し、
...
**実装済**(`examples/highorder4/`): 数学的な Laplacian は通常どおり
`def Δ u = divg (grad u)` と書き、離散化だけをmodel profileで指定する。

```formurae
discretization collocated derivative 2 centered accuracy 4
```

Egison が `divg (grad u)` を二階の `FieldJet` へまとめた後、`post-fec` の
exact-rational Taylor solverが要求精度を満たす最小半径を選ぶ。その結果、5点4次の
第2微分係数(−1/12, 4/3, −5/2, 4/3, −1/12)が得られる。
`Δ4`や `∂'` という別の数学演算子は定義せず、wideな一階差分を二重適用しない。
