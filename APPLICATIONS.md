...
| 2b | 周期領域の弾性波（直交座標と非直交座標） | 同じ v–σ 定式化を `metric tensor` の一般計量で | 計量・逆計量・計量微分を init で係数場に凍結、同一点配置、Verlet 法 | P/S 波速（平均半径の傾き）・平面波厳密解への 2 次収束・修正エネルギー保存・二座標系の一致・ブロッキング/MPI 一致（検証量はすべて `.fme`） | **済** ([例](examples/elastic_pulse/README.md)) |
