...
Formura 2.x + 現行の `pre-fec -> Egison -> FEIR -> post-fec` に乗るのは
次を満たす問題:
...
`post-fec` が格子配置、差分係数、補助場を決定する。したがって
**多項式的な非線形項(u·v², |ψ|²ψ など)はそのまま書ける**し、
`extern` 宣言した exp/cos などの関数呼出しも FEIR から Formura へ保持される。
長距離相互作用や全格子同期は引き続き Formura 側の機構を必要とする。

凡例 — ◎: 現行機能で今すぐ / ○: FEIR、`post-fec`、または Formura の小拡張で可 /
△: Formura 本体拡張(境界条件 or 大域リダクション、[UPSTREAM.md](UPSTREAM.md))後に実用化

「済」のうち `mhd_ot`、`lbm_d3q19`、`acoustic3d` は手書き Egison の
direct Egison example。`.fme` の例はすべて上記の FEIR pipeline で生成する。
...
| 11 | 曲線座標・曲面上の拡散(計量つき) | ∂t u = (1/√g)∂i(√g g^{ij}∂j u) | Egison が `GeometryNF` を構成し、`post-fec` が保存流束へlowering | 参照実装と 3.3e-16・Σ√g·u 保存 2.5e-15・最大値原理 | **済** (examples/metric_torus) |
...
| 16 | 高次スキーム自動導出(横断機能) | 通常の∆にmodel profileで 4 次精度を指定 | `post-fec` のexact Taylor solverが最小半径の係数を導出 | 導出=既知公式・離散シンボル 4.4e-16・h⁴ 残差則 98.6% | **済** (examples/highorder4) |
| 17 | 球面**全域**の拡散(Yin-Yang overset 格子) | ∂t u = Δ_{S²} u、合同 2 パネル+相互補間境界 | embedding 計量(済)+ドライバ層 overset 交換(1 kernel 2 instance) | Y₁⁰ 固有関数減衰 e^{−2t} と 7.3e-4・重なり整合 4.6e-4・最大値原理厳密 | **済** (examples/yinyang_diffusion、V0) |
...
連続系だけを表層に書き、配置と stencil を `post-fec` に決めさせる:
...
`map` で自動生成する。この例は現在も手書き Egison の direct path だが、
equilibrium の u·e_k, u² は全部多項式なので FEIR のscalar normal formでも表現できる。
「式の本数が多い系ほど得をする」ことの定量デモ(生成行数/記述行数の比)になる。

### 8. Cahn–Hilliard — 4 階微分と 2 段参照

μ を `local` として書くと、`post-fec` が .fmr の中間格子場として保存し、
...
`post-fec` が half-cell の係数場、flux、volume除算、更新順を決めて .fmr を出す。
...
`post-fec` は ca/cb/sg 等の係数場を必要な配置で初期化し、f1/f2 を半セル位置に置く
...
Egison がgeometryのない`Δ u`を二階の `FieldJet` へまとめた後、`post-fec` の
...
**compiler・fork とも無改造**。overset 合成(パネル 2 インスタンス+縁 1 セルの
相互双一次補間)は check driver が担い、対合性により補間表は両方向で同一の 1 本。
検証は厳密固有関数 Y₁⁰ の減衰 e^{−2t}(7.3e-4)、重なり領域の Yin/Yang 整合
(1460 点で 4.6e-4)、離散最大値原理(双一次の凸性により違反**厳密に 0**)。
表層宣言化(`grid yinyang margin 2`)・MPI 分割・ベクトル場成分回転・浅水/mantle
対流への展開は [design/20260714-yinyang-overset.md](design/20260714-yinyang-overset.md) の
Phase 1–4 を正とする。
...
Makefile ターゲットが `pre-fec -> Egison -> post-fec -> Formura -> C check` を通す。
