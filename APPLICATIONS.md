...
| 12 | Kuramoto–Sivashinsky(時空カオス) | ∂t u = −u∂x u − ∇²u − ∇⁴u | 4 階 = lap∘lap(中間場で半径 1×2 段) | Lyapunov 内で参照と 3.6e-14・アトラクタ統計・Σu 5e-12 | **済** (examples/ks3d) |
...
| 16 | 高次スキーム自動導出(横断機能) | Taylor 展開の連立を CAS で解き 4 次差分係数を導出 | `taylorStencil`/`dTaylor`(lib 入り) | 導出=既知公式・離散シンボル 4.4e-16・h⁴ 残差則 98.6% | **済** (examples/highorder4) |
...
**実装済**(`lib/fmrgen.egi` + `examples/highorder4/`): `gaussSolve`(厳密有理
ガウス消去)+`taylorStencil m ks`(Taylor 条件の連立を解く)+`dTaylor`
(任意次数・任意ステンシルの微分コンビネータ)。5点4次の第2微分係数
(−1/12, 4/3, −5/2, 4/3, −1/12) と第1微分係数 (1/12, −2/3, 0, 2/3, −1/12) の
導出を確認。検証は単一 Fourier モードで、生成コードの振幅比 = 導出ステンシルの
