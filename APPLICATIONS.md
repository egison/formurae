...
[円筒・球面のテンソルデモ](examples/tensor_demos/README.md)には，
半径方向を補強した異方性弾性体（円筒と球殻），Oldroyd-B 粘弾性流体の回転・停止，
球面液晶の配向緩和を実装しています．テンソルの空間演算は Formurae が C へ生成し，
粘弾性流体では時間積分・境界値も `.fme` から C に生成し，Poisson 方程式も C で解きます．
利用者は `.fme` 一つを書き，`formurae run` で実行できます．
弾性体・液晶は時間積分・座標間の交換を Python ドライバーと組み合わせます．
ボールを押す・管をねじる・液体をかき混ぜる操作を見せる導入動画もあります．
`make tensor-demos` で検証とすべての動画を再生成します．
