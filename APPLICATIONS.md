...
**現在の開発対象は、Formura 本体を拡張せずに実装できるシミュレーションに限る。**
以下のカタログには過去の候補も含む。Formura の拡張が必要な候補は対象から外し、
初期条件・境界処理・補助計算を含む全体を通常のコード生成経路で実装できるものを選ぶ。
MPI による領域分割と時間方向のブロッキングを活かせることも選定基準にする。
詳しい開発ルールは [AGENTS.md](AGENTS.md) を参照する。
...
長距離相互作用や全格子同期を必要とする手法は、既存の Formura で全体を実装できるか
確認し、本体への機能追加が必要なら対象に選ばない。

過去の評価の凡例 — ◎: 当時の現行機能で実装可能 / ○: 小規模な機能追加を想定 /
△: Formura 本体の拡張を想定（[過去の拡張案](UPSTREAM.md)）。
○・△は現在の着手候補を意味しない。◎や「済」も、新たな変更の前に上記の基準で確認する。
...
| 2b | 周期領域の弾性波（直交座標と非直交座標） | 同じ v–σ 定式化を `metric tensor` の一般計量で | 計量・逆計量・計量微分を演算子内でその場で使用（コンパイル時に展開）、同一点配置、Verlet 法 | P/S 波速（平均半径の傾き）・平面波厳密解への 2 次収束・修正エネルギー保存・二座標系の一致・ブロッキング/MPI 一致（検証量はすべて `.fme`） | **済** ([例](examples/elastic_pulse/README.md)) |
...
| 18 | トーラス上の異方的な興奮波(FitzHugh–Nagumo) | ∂t u = J⁻¹∂i(J D^{ij}∂j u) + 3u − u³ − v | 計量つき保存流束+`resample`、ねじった座標では `metric tensor`(非対角計量) | 質量保存 1.6e-11・静止状態保持・ブロッキング/MPI 一致・2 次収束(CAS 解析微分と比較)・二つの座標系で 2 次一致(比 4.00) | **済** (examples/excitable_torus) |
| 19 | トーラス上のネマティック液晶(Q テンソルの勾配流) | ∂t Q^{ij} = L∇_k∇^k Q^{ij} + (a − c\|Q\|²)Q^{ij} | Christoffel 記号を CAS が導出し係数場に凍結、テンソルの共変ラプラシアン、欠陥の巻き数を差分の恒等式で厳密計数 | 自由エネルギー単調減少・対称性厳密・トレース 1e-12・電荷中性・ブロッキング/MPI 一致・共変ラプラシアンの 2 次収束 | **済** (examples/nematic_torus) |
| 20 | 座標変換を物質に読み替える波動(変換光学: クローク・回転子) | TE Maxwell、μ^{ij} = √g g^{ij}、ε_z = √g | Yee 配置(ダミー z 軸)+極座標枠でのテンソル組み立て+`resample` | 真空で散乱 0(機械精度)・エネルギー保存・散乱率の装置比較・解像度依存・ブロッキング/MPI 一致 | **済** (examples/transformation_optics) |
...
`lbm_d3q19` はこの添字宣言を使い，密度・運動量を縮約で求め，衝突と移動を成分ごとに更新する。
