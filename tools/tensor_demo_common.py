"""Build and call actual Formurae -> Egison -> Formura -> C demo kernels.

Python owns time integration, pressure solves, panel exchange, and rendering.
...
def derivative(a, axis, h, bounded=False):
    out = (np.roll(a, -1, axis) - np.roll(a, 1, axis))/(2*h)
    if bounded:
        s = [slice(None)]*a.ndim
        lo, hi = s.copy(), s.copy()
        lo[axis], hi[axis] = 0, 1
        out[tuple(lo)] = (a[tuple(hi)]-a[tuple(lo)])/h
        lo[axis], hi[axis] = -1, -2
        out[tuple(lo)] = (a[tuple(lo)]-a[tuple(hi)])/h
    return out
...
def write_report(name, report):
    ...
    report['source_sha256'] = {str(p.relative_to(ROOT)): sha(p) for p in
                             [*sorted(SOURCES.glob('*.fme')), *sorted((ROOT/'tools').glob('tensor_demo*.py'))]}
