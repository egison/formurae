...
Status: Accepted; Phases 1-3 and 5 implemented; Phase 4 semantic kernel and Phase 6 structural `lb` planner implemented

この文書は、`.fme` の数式を `fec` が成分展開してから Egison へ渡す現在の
構成を改め、テンソル・添字・微分形式・格子配置の意味論を Egison に集約する
ための設計と移行手順を定める。
...
生成 Egison は成分別の `feqE1`、`feqE2`、... を作らず、target tensor と
tensor-valued RHS を Formura backend に渡す。

```egison
def rhsE := E + dt * curl B
def rhsB := B - dt * curl E'

def feSteps :=
  FMR.tensorEqs fePrinterContext E' rhsE
  ++ FMR.tensorEqs fePrinterContext B' rhsB
```

最初の実装は次を行う。

1. target と RHS の `tensorShape` が等しいことを検査する
2. `tensorToList` で両者を同じ順序に flatten する
3. target leaf の function symbol から基底名・添字・prime を取得する
4. field-name map を使って Formura の LHS storage 名へ変換する
5. RHS を既存の `FMR.show` で表示する

これは Formura backend helper であり、Egison core の component enumeration
primitive ではない。通常の tensor 表示は `show` で完結する。

plain tensor は全成分を zip できる。symmetric、antisymmetric、form は物理 storage
が full tensor より少ないため、field descriptor が持つ component projection を
backend が適用する。数式を成分展開する必要はなく、projection は target tensor の
flat position を選ぶだけでよい。将来、Egison の symmetry definition が同じ target
function symbol へ正準化することを利用できる場合は、target symbol の重複除去へ
置き換えてよい。
...
離散微分は target component が決まるまで評価を遅延する。equation printer が
target の concrete component indices と policy を受け取り、RHS leaf の function
symbol indices と field policy から source placement を導出して stencil を選ぶ。
...
backend は `FE.formComponents` で昇順基底だけを projection する。旧
...
FieldDescriptor =
  ...
  tensor shape
  declared variance
  symmetry / form degree
  GridPolicy
  physical component projection
```

論理 tensor shape、variance、policy は Egison evaluator と grid lowering が使う。
physical component projection、storage name、component order は Formura backend だけが
使う。`Vector Bool` / `Tensor2 Bool` や `fdStaggered :: Bool` のように layout と
placement を同じ値へ埋め込まない。
...
この phase では既存の placement lowering と rank-2 projection を変更しない。
RHS は1つの Tensor 値になったが、その各 component 式はまだ `fec` が展開して
tensor literal へ束ねている。operator evaluation 自体を Egison へ移すのは Phase 4
で行う。
...
- [ ] shape / variance / symmetry / projection を含む完全な field descriptor を生成し、
...
- [ ] Haskell の parity bits は移行検証 oracle と分岐選択にだけ残る。Phase 4 の
  derivative lowering 移管後に削除する

### Phase 4: Tensor-valued operators in Egison

- [x] `grad`、`dGrad`、`divg`、`curl`、`hessian` の tensor-valued shared Egison
  definitions と strict tests を追加する
- [ ] generated equations と user `def` を shared definitions の evaluation path へ統合する
- [ ] Haskell の `coordinatePreludeDefs`、`expandDefs`、component specialization を削減する
...
- [x] `FMR.formEqs` で whole-form equation から独立成分だけを backend projection する
- [x] maxwell_dec と `assert-dd-zero` を acceptance test にする
- [x] model ごとの d/hodge/codiff 実装と `formComps` / `formDeg` tuple helpers を削除する
- [x] grid 固有部分は generated `feFormDerivative` callback だけに縮小する
...
- [ ] vector/form 間の `flat` / `sharp` を、再構成・補間規約を定めた上で同じ geometry
  context へ統合する
...
現在の境界では、純粋な metric/Hodge/Laplace--Beltrami 公式は Egison が評価し、Haskell の
backend planner は `ca` / `cb` / `cc` / `sg` と `f1` / `f2` / `f3` の宣言・初期化・更新・
lifetime だけを計画する。保存済み flux は `feLbStoredFlux` を通して divergence から実際に参照され、
既存 metric 5例の `.fmr` と数値結果は移行前と一致する。最初の構造 plan は1モデルにつき1個の
unindexed collocated scalar source に限定し、compound source、primed source、initializer 内の
`lb` は明示診断で拒否する。`BackendRequest` は展開後 expression span を保持するが、元の `.fme`
line/column へ戻す source map は Phase 7 の未完項目である。複数 request の lifetime/scheduling と、
metric-aware form Hodge・`flat` / `sharp` は次の段階で扱う。

### Phase 7: Shrink `TensorExpr`

- strict result signature の権威を Egison `tensorIndices` に移す
- `.fme` source span と Egison expression の対応を保持する
- `TensorExpr` を surface AST / diagnostics / emitter に縮退させる
- Haskell の `ixExpand`、`strictEinstein`、operator-specific expansion を削除する
...
- projection は数式の component expansion を要求しない
