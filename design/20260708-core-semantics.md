...
Note: このメモに含まれる「添字付きテンソル演算子を `.fe` の `def` で定義する」
方針は、2026-07-09 の設計で撤回した。現行方針では、rank-changing な数式演算子は
生成 Egison 側の定義に任せ、Formurae には一般的な添字補完器を実装しない。
詳しくは `design/20260709-egison-style-indexed-defs.md` を参照する。
...
- 添字付きテンソル演算子をユーザが `.fe` 側で定義できるようにする
...
- 既存の `.fe -> .egi -> .fmr` 生成結果が、記法を変えていない例では
  できるだけバイト一致する。
...
次の大きな表層機能は、添字付き引数と添字付き返り値を持つ演算子定義である。
...
最初の実装では、完全な型システムは不要である。`.fe` の定義を Egison の
添字付きテンソル関数定義へ渡す薄い変換でよい。

実装メモ:

- `X~i`、`X_i` のような添字付き引数と、`u` のようなスカラー引数を parse する。
- `grad u_i` のような添字付き返り値形式を parse する。
- 呼び出し時には、テキスト置換ではなく Egison の添字付き関数の仕組みへ接続する。
- 既存の単純な `def NAME ARG = EXPR` は、新経路で置き換えられるまで互換性のため残す。
