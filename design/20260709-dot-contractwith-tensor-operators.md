...
`grad` や `∂ v` のように新しい自由添字を持つテンソルを返す演算子は、
結果添字つき `def` ではなく、テンソル値を返す primitive/prelude 演算子として設計する。
...
定義内に明示的な添字を書く場合、それは関数内部で縮約される局所添字に限る。
その縮約は `contractWith` または `.` によって明示する。
...
`div` の `i` は `contractWith (+)` で、`Δ` の `i`、`j` は `.` の内側の
`contractWith (+)` で縮約されて消える。
`grad` や `∂ v` のように新しい自由添字を持つテンソルを返す演算子は、
結果添字つき `def` ではなく、テンソル値を返す primitive/prelude として別途設計する。
...
3. `.` を `contractWith (+)` へ正規化する
4. index reduction を行い、同じ上下添字を supersubscript 対角として残す
5. 対象 field の自由添字ごとに成分式を生成する
6. `contractWith reducer` が supersubscript 軸を指定された reducer で畳み込む
7. 最後に `∂_i`、metric、staggered 配置を Formura stencil へ落とす
...
`grad` のように新しい自由添字を作る演算子は、結果添字つき `def` ではなく、
テンソル値を返す primitive/prelude として別途設計する。
