...
step:
  ...
  s'~i~j = s~i~j + dt * (λ * g~i~j * ∂_k v'~k
                       + μ * (g~i~k * ∂_k v'~j + g~j~k * ∂_k v'~i))
```

ここで `g` は `metric g` で宣言されたユークリッド計量であり、
`g~i~j` は反変計量として扱う。Kronecker delta は metric とは別概念として残す。
...
Kronecker delta は mixed identity として metric とは別に残す。
`δ~i_j` や `δ_i~j` は mixed identity を表す。ユークリッド座標でも
計量テンソルを表したい場合は `δ~i~j` ではなく `g~i~j` を使う。

この分離により、Kronecker delta と metric delta が同じ名前になる問題を避ける。
...
- metric は標準的に `metric g` と書き、Kronecker delta `δ` とは分ける
