...
FEIR encoderとopaque bridgeだけが使うcompiler-private bindingである。pure operator bodyへは、
...
`OperatorContext` tuple、`feOperatorContext`、`FormuraeInternalContext`は生成しない。dimensionやgeometryを
必要とするshared helperにはmodel-local operator definitionがtop-level bindingを直接渡し、user callと
higher-order valueには通常のcontext-free Egison functionを渡す。`variableMetric`はruntime flagにせず、
pre-fecがconstant-metric版またはmetric opaque版の定義を生成時に選ぶ。geometry IDとmanifest IDも
opaque constructorだけが直接参照する。model-local operator definitionは実際に参照されるものだけを生成し、
...
通常のEgison関数として定義する。shared library helperは必要な値だけを直接引数に取り、model-local
definitionが一度だけ束縛する。以下は実装と同じmathematical coreの要約である。型とindex orientationは
focused testで固定する。
...
```egison
def FormuraeInternalDiff f := Formurae.diff coordinates f
...
def FormuraeInternalCurl X :=
  (withSymbols [i, j, k]
        (epsilon_i~j~k . ∂/∂ X_k coordinates~j))_formuraeTensorIndex1

def FormuraeInternalLap u :=
  FormuraeInternalDivg (FormuraeInternalGrad u)
```
...
|---|---|---|
| `grad` | `Formurae.grad coordinates u` | ambient座標を一度束縛する1行 |
| `dGrad` | `Formurae.dGrad coordinates X` | 入力varianceを保つ1行 |
| `divg` | `Formurae.divg coordinates X` | Egison nativeの1行 |
| `curl` | `epsilon_i~j~k . ∂/∂ X_k coordinates~j` | Levi-Civita縮約の2行 |
| `hessian` | `Formurae.hessian coordinates u` | ambient座標を一度束縛する1行 |
| Cartesian `lap` | `FormuraeInternalDivg (FormuraeInternalGrad u)` | pure compositionの1行 |
...
```egison
def Formurae.codiff coordinates dimension geometryScales A :=
  let degree := dfOrder A
   in if degree = 0
        then 0
        else (-1) ^ (dimension * (degree + 1) + 1)
             * Formurae.hodge dimension geometryScales
                 (Formurae.d coordinates
                   (Formurae.hodge dimension geometryScales A))
```

variable-metric modelではpre-fecが上の定義をruntime分岐させず、`geometryId`、`dimension`、
`primitiveManifestId`だけを直接取る`metricCodiff` wrapperを生成する。
...
definitionは`Formurae.lbOrthogonal feGeometryId fePrimitiveManifestId value`であり、source policyはv1の
...
- 当初uniform OperatorContext dictionary passingでordinary/higher-order callを検証し、production cutover後に
  model-local ambient bindingsへ置換する。現行生成物にhidden context引数は残さない。
