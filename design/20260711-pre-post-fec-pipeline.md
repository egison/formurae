...
- nondefault radius / wide coordinate derivative
...
`∂'`などのradius/stencil-family指定は通常のFieldJet metadataに混ぜない。product/chain ruleと
Schwarz canonicalizationがscheme指定を消してしまうため、versioned coordinate-derivative
requestとして保持する。
...
data DiscretizationProfile = DiscretizationProfile
  { profileId :: VersionedProfileId
  , collocatedFamily :: StencilFamily
  , staggeredFamily  :: StencilFamily
...
10. lowered/implicit interpolation、grid offset、array reference、concrete stencil coefficient、storage名は
    存在しない。明示interpolation requestはOpaqueDiscreteとして存在できる。
...
- Collocatedのsingle-axis order `m`はradius `max(1, ceil(m/2))`のsymmetric exact Taylor stencil。
- Primal/Dualの各axis order 1/2はYee first derivativeまたはcompact second derivative。
- Primal/Dualのper-axis order 3以上はv1 defaultで拒否し、explicit opaque requestを要求する。
- mixed multi-indexはaxisごとのcanonical stencilをfixed axis orderで合成する。
- boundary modificationはFormura側の責務であり、post-fecはinterior stencilだけを生成する。
