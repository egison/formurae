...
Status: Implemented through Phase 10a. Public ambient geometry bindings and the
pure raw-Egison fallback are in production; Phase 10b (one direct body path for every
user `def`) remains planned. Validation records are sections 13 and 14.4.
...
runnerはそのlibrary群とgenerated normalization unitをEgisonの同じinitial load batchに入れる。
Egisonはbatch内の全top-level definitionを一度に型検査してrecursive bindするため、
shared `Formurae.*`演算子はgenerated unitが後ろで定義するambient bindingを直接参照できる。
normalizationは1 invocationにつき1 model unitとし、そのrecursive batch構築後にunitの`main`を
明示的に呼び出す。
...
- user `def`を通常のEgison functionとしてemitする。Phase 10aでは既存surface bodyとpure
  raw-Egison bodyの二経路を使い、Phase 10bで一つのdirect body pathへ統一する
...
equation、initializer、explicit discrete primitiveには`TensorExpr`相当のsurface ASTをpre-fecに
残す。ただし値をHaskellで実行するためではなく、parse、source diagnostics、effect analysis、
Egison syntax generationに限定する。pure user `def`の最終形はこの限定ASTへ落とさない。pre-fecは
indent blockを一つのEgison expressionとして収集し、Formurae固有sugar/effectに必要な最小のpassだけを
行う。`let`、lambda、`match`、`generateTensor`、tensor index expressionの文法と評価の権威はEgison
parser/evaluatorである。

現行Phase 10aでは、既存`TensorExpr`でparseできるbodyは従来のFormurae sugar/effect変換を使い、
parseできないrich multi-line bodyだけをpure raw Egison fallbackとして渡す。fallback bodyのidentifierは
宣言座標だけをcanonical coordinate nameへrenameし、それ以外は書き換えない。先行user definitionと
standard operatorは、definition-order semanticsを固定するcompiler-generated lexical `let` aliasとして
raw bodyの外側へ置く。raw fallback内のunshadowed opaque primitiveはPhase 10aでは拒否する。
structured bodyで名前が重複しない明示的な自由結果indexが残る場合は、そのvarianceをoperator名に
依存せずfunction returnへ付与し、後続の加算・縮約より前にordinary tensorとして合成できるようにする。
repeat-index/diagonalのような曖昧な結果は保守的に付与しない。raw rich bodyが
新しいfree-index tensorを構築する場合の同じ推論はPhase 10aの対象外である。Phase 10bでEgison ASTに
基づくresult contract、effect-aware bridge、Formurae derivative sugarをdirect bodyへ接続した後、
user `def`から`TensorExpr` definition pathを削除する。
...
- standard operatorの実引数とambient model bindingsに基づくrank/mode/dimension/degree前提の検査
...
### 3.5 Ambient model environment

一つのFormurae modelは一つのEgison normalization unitへ変換される。pre-fecはそのunitの先頭で
coordinate vector、dimension、geometryをtop-level bindingとして一度だけ生成する。`fe...`名は
normalization library、geometry構築、opaque bridge、FEIR encoderが使うcompiler-private bindingである。
pure user bodyへは、
Egisonのgeometry sampleと同じ役割を持つ短いpublic ambient nameを公開する。

```egison
def feDimension := 3
def feCoordinates := [| x, y, z |]
def feGeometryScales := [| h1, h2, h3 |]
def feGeometryId := 1
def fePrimitiveManifestId := "sha256:..."

def dimension := feDimension
def coordinates := feCoordinates
def metric_i_j := feGeometryMetric_i_j
def inverseMetric~i~j := feGeometryInverseMetric~i~j
def volume := feGeometryVolume
def epsilon := ε dimension
```

`dimension`、`coordinates`、`metric`、`inverseMetric`、`volume`、`epsilon`はambient bindingの
予約語であり、Formurae modelのfield、parameter、user `def`、formal parameter、step-level
`let`/`local` target、metric nameでshadowできない。一方、raw Egison body内のlexical `let`やlambdaの
formalはEgison自身の局所scopeであり、この予約規則の対象にしない。例えば補助lambdaの引数名として
`metric`を使えば、そのlambda内だけambient metricをshadowする通常のEgison semanticsになる。
座標名は従来どおり`axes`宣言から直接参照できる。Euclidean modelでも`metric`と
`inverseMetric`はidentity tensor、`volume`は1として必ず存在する。従ってpure operatorをgeometryの
種類ごとに別の定義へ分岐させない。

surfaceに`metric g`があれば、`g`という裸の値を一つ生成するのではなく、variance別のEgison
indexed-definition sugarを生成する。metric surface nameはmodelごとに高々一つだけ宣言でき、重複する
`metric NAME`は同名でも別名でもpre-fec errorとする。

```egison
def g_i_j := metric_i_j
def g~i~j := inverseMetric~i~j
```

このためuser bodyでは`g_i_j`がcovariant metricを、`g~i~j`がinverse metricを参照する。
whole tensorはEgisonのindex completionにより`g_#_#` / `g~#~#`として参照する。bare `g`、
`g~i_j`、`g_i~j`は暗黙には定義しない。mixed identityが必要ならKronecker tensorを
明示し、indexの上げ下げは`g_i_j . X~j`または`g~i~j . A_j`と式に書く。宣言名`g`は二つの
indexed definitionが占有するため、同名のFormurae field、parameter、definition、formal parameter、
step-level `let`/`local` targetはhard errorとする。raw body内だけのlexical binderはこの制約に含めない。

`metric_i_j` / `inverseMetric~i~j`は実装を単純にし、宣言名を必要としないoperator definitionも
書けるcanonical indexed familyである。whole tensorはそれぞれ`metric_#_#` / `inverseMetric~#~#`
で得る。例えば次の二式は同じ値へ正規化される。

```formurae
metric g
def raiseWithG A = withSymbols [i, j] (g~i~j . A_j)
def raiseAmbient A = withSymbols [i, j] (inverseMetric~i~j . A_j)
```

metric tensorはfieldではないため、FEIRのlogical field registryやFieldJetには追加しない。metric
componentはgeometry由来のconstant/sampleable coefficientとして式へ入り、FEIR headerの
`GeometryNF.metric` / `GeometryNF.inverseMetric`と同じsymbolic valueを共有する。

数学演算子はEgisonのgeometry sampleと同様にこのambient environmentを参照する。全user functionへ
hidden dictionaryを追加せず、概念的に次を生成する。
...
def lap2 u := FormuraeInternalDivg (FormuraeInternalGrad u)
```

`OperatorContext` tuple、`feOperatorContext`、`FormuraeInternalContext`は生成しない。public
`Formurae.*`のcontinuum operatorは数学的operandだけを引数に取り、`coordinates`、`dimension`、
`feGeometryScales`などのmodel-local値はrecursive load batch内のambient bindingから直接参照する。
opaque request constructorもoperandとoccurrence-local attributeだけを取り、geometry IDとmanifest IDは
ambientなcompiler-private bindingから参照する。user callとhigher-order valueには通常の
context-free Egison functionを渡す。`variableMetric`はruntime flagにせず、
pre-fecがconstant-metric版またはmetric opaque版の定義を生成時に選ぶ。geometry IDとmanifest IDも
opaque constructorだけが直接参照する。model-local operator definitionはgeometry dispatchと
lexical hygieneのための薄いoperand-only bridgeであり、実際に参照されるものだけを生成し、
未使用のstandard operator boilerplateをunitへ含めない。ただしPhase 10aのraw bodyはshallow dependency
scanなので、local/user shadowされたstandard名のwrapperを安全側で余分に生成する場合がある。

このambient closureはmodel-specificな`Formurae.*` APIの規則である。再利用可能な低層の
`FE.*` geometry/tensor helperは従来どおりdimension、coordinates、scale functionなどを
明示引数に取る。また`FEIR.encodeScalar`と`FEIR.encodeTensorWithMetadata`は
parameter/coordinate/field/function registryを明示的に受け取る。これらはmodel演算子ではなく、
複数の値を相手にする汎用helperまたはserialization boundaryだからである。
user shadowingは通常のlexical resolutionでstandard preludeより
優先する。surfaceの`def (.) a b = ...`も通常のcontext-free internal definitionへemitし、後続のinfix dot
chainはその関数へ左結合で解決する。user定義がなければEgison本来のtensor contraction dotを使う。
`materialize`等の明示離散primitive名も先行user definitionやbound parameterがあれば同じlexical ruleで
shadowされ、未shadowの場合だけversioned primitiveへ解決する。Phase 10aのraw body内だけはshallow
effect scanがlocal binderを理解しないため、`let materialize := ...`のような局所shadowを安全側で拒否する。
...
参照することはない。Euclidean unitは全成分1のscale vectorを束縛し、pre-fecがconstant-metric版の
演算子定義を選ぶ。
...
解析微分とtensor algebraだけから定義できる演算子は、generated unitのambient bindingsを参照する
通常のEgison関数として定義する。shared `Formurae.*`演算子はoperand-only APIを持ち、
model固有のcoordinates、dimension、geometryはgenerated unitとのrecursive bindingから取得する。
以下は実装と同じmathematical coreの要約である。型とindex orientationは
focused testで固定する。

#### 4.1.1 User `def`はEgison expression blockである

pure operatorをFormurae独自の小さな式言語で再実装しない。single-line formに加えて、`=`の次行を
headより深くindentしたblockを一つのEgison expressionとして受理する。

```formurae
def scaledByDimension a X =
  let scale := \Y -> a * Y
   in match dimension = 3 as bool with
        | #True -> scale X
        | #False -> X
```

blockは次の空でない行のindentが`def` head以下へ戻った時点で終わる。Phase 10aは空でないcode lineの
改行と相対indent、および各行のsource positionを保持する。blank lineとcommentはblock収集時に除去する。
最終形のpre-fecは概念的に次だけを扱う。

1. declaration間のcollisionとambient予約語を検査し、先行user definitionとstandard operatorを
   definition-orderを保つlexical bindingとして解決する。
2. `∂_i X_j`、`∂_x u`等のFormurae derivative sugarを`∂/∂`を使うEgison式へ置換する。
3. unshadowedなexplicit discrete primitiveをversioned opaque constructorへ置換し、transitive effect
   summaryを記録する。

それ以外の`let`、lambda、`match`、`withSymbols`、`generateTensor`、関数適用、tensor/index構文は
同じ構造とindentでgenerated `.egi`へ渡す。Phase 10aがraw body内で直接renameするのは宣言座標だけで、
先行user definitionとstandard operatorはbodyの外側にcompiler-generated lexical `let` aliasとして
置く。このためlambda/`let` binderのshadowingをpre-fecが再実装する必要はない。pre-fecがrich syntaxを
`TensorExpr`へ変換したり、operator名からresult rank/varianceを推論したりしない。

Phase 10aのraw definitionに由来するEgison parse/type/evaluation errorは、consumer equationのlocationと
definition block全体のrangeをexpansion frameとして示す。raw body内部の正確なbranch line/columnと
nested raw-definition traceは、Egison AST/source mapを接続するPhase 10bの課題である。

Phase 10aではこのdirect pathをpure raw fallbackとして導入する。`TensorExpr`でparseできる既存bodyは
従来pathを通るためFormurae derivative sugarとopaque primitiveを引き続き使える。rich syntaxによって
raw fallbackへ入ったbodyはEgison native syntax、ambient binding、生成されたlexical aliasだけを使う
pure expressionに限り、unshadowed opaque primitiveを含めば明示診断にする。この制限はsilentな
effect lossを防ぐためであり、Phase 10bでraw body用effect bridgeを実装してから取り除く。

Phase 10aのraw fallbackで安全に合成できるtensor resultは、入力tensorをそのまま返すもの、または
`grad`、`hodge`等のmetadata付きoperator resultを`let`/`match`で組み合わせるものに限る。raw bodyが
`withSymbols`等から新しいfree-index tensorを構築する場合、そのresult variance contractをpre-fecは
まだ推論しない。これはEgison ASTを直接使うPhase 10bで統一する。

上例のgenerated coreは次の形になる。

```egison
def FormuraeInternalDefinition1 a X := withSymbols [i, j, k, l, m, n] (
  let scale := \Y -> a * Y
   in match dimension = 3 as bool with
        | #True -> scale X
        | #False -> X
)
def scaledByDimension := FormuraeInternalDefinition1
```

結果のshape、variance、`dfOrder`は実際に評価したwhole tensorとequation targetをFEIR境界で照合する。
従って新しいpure operatorをpre-fecのoperator tableへ登録する必要はない。standard operatorも通常の
先行definitionとして同じlexical ruleに従い、userが同名を先に定義した場合はuser definitionが勝つ。

最終形ではexpression blockがpureかどうかを構文の狭さではなくeffect summaryで判定する。
`materialize`、`fluxDiv`、`lb`等を含むdefinitionはsummaryが`Discrete`となり、その値をanalytic
derivativeのoperandまたはpure higher-order argumentとして使うことをnormalization前に拒否する。
Phase 10aのraw fallbackはそれらのcall自体を拒否することで、同じpure/opaque境界を保守的に満たす。

#### 4.1.2 Ambient geometryを使う短い定義

ambient予約語と`metric g`のindexed aliasesにより、geometry-dependentなuser operatorもhidden
contextやFormurae library callbackなしで定義できる。
...
def codifferential A =
  let k := dfOrder A
   in (-1) ^ (dimension * (k + 1) + 1)
      * hodge (d (hodge A))
```

`raise`と`curl`はpure user definitionとして直接Egisonへ渡り、FEIRにはmetric coefficient、
Levi-Civita contraction、analytic FieldJetだけが残る。`codifferential`のような基本演算子の合成も
同じ経路を通る。variable-metric discrete adjointを要求する場合だけ、後述のopaque `metricCodiff`
または標準`codiff`のmodel-local opaque definitionを使う。

Phase 10aは`raise`のfree `~i`と`curl`のfree `_i`をstructured bodyから検出し、生成function returnを
それぞれ`Formurae.attachExplicitVariances ["up"]` / `["down"]`で包む。これは`raise`/`curl`という
operator名の特殊規則ではなく、名前が重複しないfree result indexの構造的contractである。このcontractが
あるため、`E + dt * curl B`の加算時点で両operandのaxisが一致し、equation boundaryまでanonymous axisを
持ち越さない。

```egison
def FormuraeInternalDiff f := Formurae.diff f
...
`∂/∂`は未知のanalytic derivative ruleを0へ落とさずerrorにする。
shared Formurae operatorも同じ`∂/∂`を直接使う。
...
|---|---|---|
| `grad` | `Formurae.grad u` | ambient `coordinates`をlibrary内で参照する1行 |
| `dGrad` | `Formurae.dGrad X` | 入力varianceを保つoperand-only関数 |
| `divg` | `Formurae.divg X` | Egison nativeの1行 |
| `curl` | `Formurae.curl X` | ambient `epsilon` / `coordinates`によるLevi-Civita縮約 |
| `hessian` | `Formurae.hessian u` | ambient `coordinates`を参照する1行 |
| Cartesian `lap` | `Formurae.lap u` | pure compositionの1行 |
...
variable-metric modelではpre-fecが上の定義をruntime分岐させず、operand-onlyの
`Formurae.metricCodiff A`へ接続するwrapperを生成する。`metricCodiff`本体はambient
`dimension`、`feGeometryId`、`fePrimitiveManifestId`を直接参照する。
...
shape、variance、df-orderはpre-fecのoperator名別型推論ではなく、`Formurae.materialized manifestId value`が
...
`divg V`、`E' = E + dt * curl B`のようにwhole tensorを通常のEgison関数へ渡す。`lb`のmodel-local
definitionは`Formurae.lbOrthogonal value`であり、geometry IDとmanifest IDはambientから取得する。
source policyはv1のcollocated contractからEgison constructorが設定する。
...
Egisonの`∂/∂`はFunctionData、constant symbol、登録derivative rule以外を
微分前に拒否し、unmatched applyを0へ落とさないことでinvariant 8を保証する。
...
```text
Constant       number / named constant / parameter; every placementで利用可能
...
Egisonが実際のtensor値とambient dimension/geometry bindingsを使って検査し、失敗時は同じmarkerを通して呼出し元の
`.fme`式位置へ戻す。
...
deterministic orderingだけを実装する。`NamedConstant Pi`はexact coefficientとしてfoldせずFMR ASTまで
保持し、rendererが最終backend境界でbinary64のπと同値な
`(884279719003555 / 281474976710656)`へ変換する。これにより`19π/24`のような式を2^53超の
整数operandへ早期合成しない。
...
- 当初のOperatorContext dictionary prototype、opaque barrier/semantic key、primitive manifest、
  provenance sidecarをfocused prototypeで検証する。OperatorContextは後のambient environment移行で削除した。
- GeometryNF、raw initializer、default discretization profile、analytic differentiationをfixtureで固定する。
...
- exact rational、named constant、ID、origin、TensorNF、FieldJet、OpaqueDiscreteをround-tripする。
...
- UTF-8名とescape
- exact rational
- named constantと未知constant tagの拒否
...
### Phase 4（完了）: analytic/opaque derivative環境仕様

- 当初uniform OperatorContext dictionary passingでordinary/higher-order callを検証し、production cutover後に
  model-local ambient bindingsへ置換する。shared libraryとgenerated unitを同じrecursive load batchに置き、
  public `Formurae.*`はoperand-onlyにする。現行生成物にhidden context引数は残さない。
...
- Formuraeのanalytic differentiationが未登録の微分規則をerrorにすることをEgisonで固定する。
...
- multi-line user `def`の`let`、lambda、`match`が構造を保った通常のEgison functionになること
- ambient `dimension` / `coordinates` / `metric` / `inverseMetric` / `volume` / `epsilon`の解決
- `metric g`の`g_#_#` / `g~#~#` indexed-definition aliasとvariance
...
- multi-line pure user operatorからFieldJet/metric coefficientを持つFEIRへの変換
- rich syntaxを含むpure user operatorとopaque effectful operatorの分離
- diffusion 1D/2D/3D
- collocated Maxwell
- Yee Maxwell
...
Phase 10aはrich pure bodyのfallbackとambient geometryを実装済みである。本改良全体はPhase 10bを含む
次をすべて満たしたとき完了とする。

- standard pure Formurae operatorの数学的定義がshared Egison libraryにのみ存在する。
- userが`let`、lambda、`match`、native tensor/index syntaxを含むpure operatorを、Egisonと同じ
  1行または数行のbodyで定義できる。
- geometry-dependentなuser operatorがambient予約語または`metric g`の`g_i_j` / `g~i~j`だけで
  定義でき、hidden contextを必要としない。
...
短い定義にする」という要求はPhase 10aのpure raw fallbackで実用可能になり、Phase 10bのbody-path
統一でFormurae固有sugarを含むすべてのuser definitionについて完成する。
...
| Egison core | `../egison/CLAUDE.md`の代表7 sample、mini-test全件、`gtimeout 600 cabal test` | analytic derivative error handlingと`symbolIndices`変更がEgison全体に回帰を入れない | representative 7/7、mini-test 117/117、cabal test 29/29。warning/error scanも空 |
...
## 14. Phase 10: Direct pure-operator bodyとambient metric

Phase 0--9ではstandard operatorの合成を通常のEgison functionへ移したが、user `def`のbodyは
なおFormurae独自の限定`TensorExpr`を経由していた。Phase 10はこの最後の非対称を取り除く。

### 14.1 現行Phase 10aとPhase 10bの実装境界

1. **Phase 10a完了:** single-line bodyに加えて、`def NAME ARGS... =`の後のindent blockを収集する。
   空でないcode lineの改行・相対indentとsource position mapを保持し、次のtop-level宣言、`init:`、
   `step:`までを取り込まない。blank lineとcomment自体はgenerated bodyへ残さない。
2. **Phase 10a完了:** 既存`TensorExpr`でparseできないrich bodyをpure raw Egison fallbackへ回す。
   宣言座標だけをcanonical nameへrenameし、先行user definitionとstandard operatorはbody外側のlexical
   `let` aliasへ固定する。unshadowed opaque primitiveは明示的に拒否する。
3. **Phase 10a完了:** structured user definitionに名前が重複しない明示的なfree result indexがあれば、
   そのvariance列を構造的に推論してfunction returnへ付与する。これによりuser-defined `curl`等をwhole
   tensorの加算・縮約へそのまま合成できる。repeat-index/diagonalは保守的に対象外とし、operator名別
   result tableは使わない。
4. **Phase 10a完了:** generated unitにpublic ambient binding `dimension`、`coordinates`、
   `metric_i_j`、`inverseMetric~i~j`、`volume`、`epsilon`を追加し、Euclidean modelを含め常に
   同じ名前とtensor varianceを与える。`epsilon`は使用有無にかかわらずambient値として生成する。
5. **Phase 10a完了:** `metric NAME`をcovariant/inverse二つのindexed definitionへemitする。
   `NAME_#_#` / `NAME~#~#`はEgison index completionでwhole tensorを参照する。
6. **Phase 10a完了:** ambient予約語を全Formurae model binding siteで拒否する。一方、raw Egison
   body内のlexical `let`/lambda binderは通常の局所shadowを許す。`curl`、`hodge`等のstandard
   operator名とexplicit primitive名の既存definition-level shadow ruleも維持する。
7. **Phase 10a完了:** raw fallbackを`Pure`に限定し、surface alias、qualified `Formurae.*` bridge、
   internal bridgeまたはtransitive user definitionを通したopaque callをsilentにpure扱いせず拒否する。
8. **Phase 10a完了:** multi-line `let` / lambda / `match`のvertical fixture、raw effect/trace、ambient
   metric emitter、scale metricのFEIR/FMRをfocused testへ追加し、全generated exampleとgalleryを
   ambient environment仕様で再生成する。`maxwell3d`は組込み`curl`名の特別扱いに頼らず、
   Levi-Civita contractionからなる短いuser operatorをmodel内で定義する。
9. **Phase 10b:** derivative/effect/result-contract analysisをEgison ASTを使う一つのdirect body pathへ
   接続し、user `def`から`TensorExpr` definition pathを削除する。raw definition内部の正確なsource
   location/nested traceと、conservative dependency scanによる未使用standard wrapperのover-emissionも
   ここで解消する。
10. **Phase 10b:** rich user operatorを通常example/galleryへ追加し、direct bodyだけのartifact
   determinism、opaque separation、source diagnosticsを最終検証する。

後方互換shimは追加しない。既存のsingle-line `def`は新しいexpression-block表現の長さ1のケースとして
残る。Phase 10aのdual body pathは現行production構成であり、Phase 10b完了時に旧`TensorExpr`
definition emitterを削除する。旧pathを恒久fallbackやcompatibility modeとしては残さない。

### 14.2 Normative input / generated Egison / FEIR tests

#### Multi-line `let` / lambda / `match`
...
def scaled a X =
  let apply := \Y -> a * Y
   in match dimension = 2 as bool with
        | #True -> apply X
        | #False -> X
...
def scaled := FormuraeInternalDefinition1
```

FEIR RHSは通常のscalar multiplication（canonicalization後の`2 * Field(u)`）で、`match`、lambda、
user function headは残らない。Phase 10aのdiagnosticはconsumer equationとdefinition block rangeを示し、
branch内部の正確なline/columnはPhase 10bで追加する。

#### `metric g`のcovariant / inverse views
...
generated environmentは少なくとも次を含む。
...
FEIRの`X~1`は`(1/4) * A_1`、`X~2`は`(1/9) * A_2`へ正規化され、target metadataは
`shape [2] / variance up / dfOrder 0`である。dual testとして`g_i_j . X~j`は係数`4, 9`を持つ
covectorになる。`g_#_#` / `g~#~#`は対応するwhole tensorを返す。`g`のbare useとmixed
`g~i_j`はnormalization error、`inverseMetric~i~j`を直接使う
宣言なしの式は同じFEIRになることも固定する。

#### Reserved ambient names

次を個別のnegative fixtureにする。

| 入力site | 例 | 結果 |
|---|---|---|
| `def` | `def dimension X = X` | declaration lineを示すpre-fec error |
| formal | `def f coordinates = coordinates` | parameter locationを示すpre-fec error |
| field/parameter | `field epsilon` / `param volume = 1` | declaration error |
| step-level `let` / `local` target | `let inverseMetric = u` | target locationを示すscope error |
| metric name | `metric metric` | canonical ambient名とのcollision error |
| duplicate metric name | `metric g`の後の`metric h` | 2つ目の宣言位置を示すpre-fec error |

失敗時stdoutは空で、internal constructor名をdiagnosticへ露出しない。
raw Egison body内の`let metric := ...`と`\epsilon -> ...`はpositive fixtureとし、局所scopeを出た後は
ambient bindingが再び見えることを検査する。

#### User shadowing

standard operatorは予約語にしない。

```formurae
def curl X = X
def useCurl X = curl X
```

`useCurl`は先行user definitionを参照する。structured bodyでは未使用のstandard curl wrapperを生成しない。
`epsilon`はoperator dependencyではなくambient予約値なので常に生成する。Phase 10aのraw body dependency
scanは安全側に倒すため、user/local bindingがstandard operatorをshadowしていても未使用wrapperを余分に
生成することがあるが、lexical `let` aliasの優先順位により意味は変わらない。Phase 10bではEgison ASTの
scope情報からこの保守的なover-emissionも除く。lambda/`let` binderによる局所shadowはEgisonのlexical
scopeに従い、pre-fecの単純なtoken置換でstandard/user bindingへ誤解決しない。

explicit primitive名にも同じ既存規則を適用する。
...
これはpureでありFEIRに`operator.materialized@1`を生成しない。先行user definitionがない同じcallだけが
opaque constructorになる。

#### Pure / opaque separation

Phase 10aでopaqueを使うdefinitionは既存のparse可能なexpression pathを使う。

```formurae
def shifted X = materialize (X + 1)
```

これはeffect summaryが`Discrete {operator.materialized@1}`となり、FEIRはoperand `X + 1`を持つ
一つのopaque request groupを生成する。post-fecはmaterialized auxiliaryをconsumerより前にscheduleする。
対してrich raw fallbackにopaque callを置いた次の入力はPhase 10aでは拒否する。
...
診断は「direct Egison definitionはcontinuum-pureでなければならない」こと、definition context、
検出したversioned operationを示し、stdoutとFEIRは空である。Phase 10aのshallow identifier scanは
callを`let`、lambda、`match`の内側に置いても見失わないが、raw local binderによるopaque名のshadowも
安全側で拒否する。正確なcall位置とlocal binderを理解するeffect analysisはPhase 10bで追加する。
また既存effect pathでは

```formurae
def invalid X = ∂_x (shifted X)
```

はanalytic derivativeがtransitively discrete definitionを含むため、`shifted`のdefinition/call traceを
持つpre-fec errorとなりFEIRを出力しない。

Phase 10bではraw body用effect bridgeを追加し、上のmulti-line `let`版`shifted`もsingle-line版と同じ
opaque FEIRへ変換する。その時点でも`invalid`の拒否は変えない。

### 14.3 完了条件

Phase 10aの完了条件:

- rich pure user operator bodyの受理可否が`TensorExpr`のconstructor集合に依存しない。
- raw fallbackのdirect/transitive opaque callを明示的に拒否し、FEIRを出力しない。
- `metric g`とcanonical `metric` / `inverseMetric`が同じgeometry symbolic valuesを参照し、
  `g_#_#` / `g~#~#`が正しいvarianceとmetric coefficientでFEIR/FMRまで到達する。
- ambient名とmetric宣言名のFormurae model binding collisionを一貫して拒否し、raw body内の通常の
  lexical binderは許す。
- 全exampleとgallery artifactが新しいambient environmentを持つgeneratorから再生成される。

Phase 10bの完了条件:

- 全user `def`をdirect body pathへ統一し、`TensorExpr` definition emitterを削除する。
- rich body内でもFormurae derivative sugarとopaque bridgeを使え、同じeffect contractで検査される。
- rich bodyが新しく構築するfree-index tensorにもEgison ASTからresult variance contractを付与できる。
- raw definition内部の正確なsource locationとnested expansion traceを診断できる。
- lexical scopeに基づくdependency analysisで未使用standard wrapperを生成しない。

### 14.4 Phase 10a検証記録

| 境界 | test / artifact | 固定する内容 |
|---|---|---|
| parse / emit / Egison / FEIR / post-fec | `tests/fixtures/pre_fec_raw_operator.fme`と`tests/pre_fec_pipeline.sh` | multi-line `let`、lambda、複数armの`match`、ambient名の局所shadow、standard `lap`のlexical aliasを正規化し、user function headを残さずFMRへ到達 |
| effect / trace | `tests/pre_raw_egison_fallback.hs` | prior pure definition、forward reference、direct/qualified/transitive opaque拒否、raw definitionのblock-range expansion frame |
| ambient emit | `tests/pre_ambient_metric.hs` | Euclidean/scale/embeddingの`dimension`、`coordinates`、`metric_i_j`、`inverseMetric~i~j`、`volume`、`epsilon`と`metric g` aliases |
| metric vertical | `tests/formurae_metric_tensor_ops.fme`と`tests/pre_fec_pipeline.sh` | scale `[1,2]`から`g_#_# = diag(1,4)`、`g~#~# = diag(1,1/4)`、およびvector contractionをFEIR/FMRで検証 |
| namespace diagnostics | `tests/pre_static_diagnostic_cli.sh` | ambient coordinate/formal、reserved metric name、metric nameとfieldのcollisionをstdoutなしのpre-fec errorにする |
| metric access diagnostics | `tests/pre_egison_diagnostic.sh` | bare `g`とmixed `g~i_j`をEgison normalization errorにし、FEIRを出力しない |
| regression / artifacts | `examples/maxwell3d`、`tests/compiler_suite.sh`、全generated `.egi`、`gallery/index.html`、`gallery/dsl/index.html` | free-index result contractを持つMaxwellの短いuser-defined `curl`、focused testのcompiler suite組込み、全modelのambient environmentを反映 |

Phase 10aは意図的にdual body pathである。rich raw body内のFormurae derivative sugar、opaque primitive、
新規free-index tensorのresult contract、正確なbranch-level diagnostic、nested raw trace、scope-awareな
unused-wrapper除去は未実装であり、上のPhase 10b完了条件に属する。
