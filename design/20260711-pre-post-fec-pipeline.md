...
shape一致、匿名軸をdownとしたlogical variance一致の場合だけ、その匿名軸をexplicit down indexへ補完する。
これはrankと実際のindex数に基づく構造的index completionであり、operator名を見ない。up targetへ匿名down
軸を読み替えることはなく、form targetのpositive `dfOrder`も保持する。このため次の通常定義をそのまま
ordinary covector fieldにも使える。
