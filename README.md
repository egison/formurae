...
| `examples/metric_torus/` | **計量つき拡散(トーラス上の Laplace–Beltrami)**(CAS が計量から √g, g^{ij} の係数場を記号導出し、init 本体まで生成する看板デモ) |
...
| `gallery/` | **全応用例の可視化ギャラリー**(`gen.sh` → `tools/render.py` → `index.html`。外部ライブラリ不要 — PNG/SVG を標準ライブラリだけで生成。ブラウザで `gallery/index.html` を開く) |
