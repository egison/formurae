...
| パス | 内容 |
|---|---|
| `tools/fec.py` | **.fe コンパイラ(DSL v1)**: 表層 DSL(`field E : vector`・`E'_i = E_i + dt * (curl B)_i`・`B' = B - dt * d E'`)を埋め込み形 .egi に変換。意味論は Egison 側に一本化(依存ゼロの薄い変換層) |
