...
- **Maxwell**(128×16×16、dt = 0.1dx、100 step): エネルギードリフト 0.33%、
  +x への輸送は2ランク実験のランク間エネルギー移動で確認(位置ベースの計測は
  既知問題⑥の per-variable cursor のため不可)。生成式は手計算の curl と符号・係数一致。
...
3. **Formura 2.3.2 の temporal blocking は袖幅2ステンシルで halo を誤処理する**(確定):
   ...
   Yee スキーム(袖幅1)なら回避できる(maxwell3d_yee で TB あり/なしのビット一致を確認済み)。
   上流への最小再現の報告は今後の課題。
...
6. **per-variable cursor と単一 offset の不整合**(Formura 2.3.2): 変数ごとのステンシル形状が
   異なると、同一ステップ内でも変数ごとに配列内シフト量が変わる(collocated Maxwell では
   E 族 +3/step・B 族 +2/step をデルタ応答テストで確認)。一方 `Formura_Navi` の offset は
   1軸1個なので、`to_pos_*` が全変数に対して同時に正しくなり得ない。多変数プログラムの
   位置読み出しをする前に「1点インパルス→1ステップ→応答位置」のデルタ応答テストで
   各変数のシフトを実測すること。Yee 例は全6場が同一シフトなので `to_pos_*` で正しい。
