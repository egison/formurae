...
```
.fe      : 表層 DSL(field 宣言+添字記法・微分形式・計量の方程式) ← 15例が移行済
...
| `gallery/dsl/` | **DSL ギャラリー**(**.fe 14例**+v0 埋め込み1例(弾性波)の15カード。各カードは .fe → 中間 .egi → 生成 .fmr の3段表示。移行は .fmr 比較(バイト一致 or 整形差のみ)+全チェックで検証) |
