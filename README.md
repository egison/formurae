...
表層言語は **Formurae**(フォーミュレ、拡張子 `.fme`)と名づけた。Formura のラテン語風複数形で、
*formulae*(数式)への掛詞 — 数式(formulae)をそのまま書けば Formura が走る。
Formura を設計した村主崇行氏への敬意を込め、氏の言語の名前を継いでいる。

```
.fme      : Formurae — 表層言語(field 宣言+添字記法・微分形式・計量の方程式) ← 23例
   ↓        fec(構文・診断+未移行の TensorExpr lowering・backend planning、Haskell; cabal build)
Egison   : 生成 .egi + grid/tensor/geometry/runtime ライブラリ(CAS・DEC・計量・Formuraプリンタ)
...
```
E_down2' = E_down2[i,j,k]
          + B_down3[i-1,j,k]*dt/(2*dx) - B_down3[i+1,j,k]*dt/(2*dx)
          - B_down1[i,j,k-1]*dt/(2*dz) + B_down1[i,j,k+1]*dt/(2*dz)
```
...
make diffusion3d  # .fme → fec → Egison → Formura → cc → 実行(質量保存を検査)
make maxwell3d    # 同上(エネルギー保存・伝播を検査)
```

## 関連論文

- **Formurae paper draft**: ワークスペース内の [`../formurae-paper/main.tex`](../formurae-paper/main.tex)。
  Formurae の表層言語、TensorExpr lowering、Egison から Formura への生成、26例の検証をまとめている。
- **Formura**: Muranushi らの FHPC'16 論文
  [Automatic Generation of Efficient Codes from Mathematical Descriptions of Stencil Computation](https://github.com/nushio3/formura/blob/master/reference/muranushi%2B-fhpc2016.pdf)。
  Formurae が出力先としている Formura の設計背景。
- **Egison tensor index notation**: ワークスペース内の [`../type-tensor-paper/main.tex`](../type-tensor-paper/main.tex) /
  [`../tensor-paper5/main.tex`](../tensor-paper5/main.tex)。
  `withSymbols`、`contractWith`、ユーザ定義可能な `.`、テンソル演算子を通常の関数として扱う設計の背景。
...
| `examples/maxwell3d_yee/` | **Yee-FDTD**(`E_i @ primal` / `B_i @ dual` と成分添字 parity から E=辺・B=面の配置を導出し、leapfrog FDTD を生成) |
...
| `examples/elastic3d/` | **弾性波(Virieux)**(.fme の **添字記法2行**: `field v~i @ primal`、`field σ{~i~j} @ primal` と宣言し、`v'~i = v~i + (dt/ρ0) * contractWith (+) (∂_j σ~i~j)`、`σ'~i~j = … λ * g~i~j * contractWith (+) (∂_k v'~k) … g~i~k . ∂_k v'~j …` と書く。繰り返し上下添字は `contractWith` または `.` で明示的に縮約し、上げ下げは metric を明示する。`@ primal` policy と成分添字の parity から ∂_a の半セル差分 = Virieux 格子を導出。P/S 両波速を1回で実測) |
| `examples/metric_torus/` | **計量つき拡散(トーラス上の Laplace–Beltrami)**(.fme の `embedding [...]`(座標系の埋め込み)だけから CAS が計量 g_ab=∂X·∂X を導出・**直交性を記号検査**・h_a=√g_aa → hodge 因子の係数場・半セル評価・保存流束まで自動。`def Δ u = lb u` と定義し、物理は `u' = u + dt * Δ u` の1行。`metric scale` 直接指定も可) |
...
| `examples/metric_sphere/` | **球面の帯(正曲率)**(.fme の `embedding` から g=diag(1, sin²θ) を CAS 導出、θ 壁は mirror。参照実装と 5.6e-16・Σ√g·u 保存 3.6e-15) |
...
| `gallery/usage.html` | **使い方ガイド**(セットアップ → チュートリアル → .fme/yaml リファレンス → 検証ドライバの書き方 → 3層構造の考え方。ブラウザで開く) |
| `gallery/dsl/` | **Formurae ギャラリー**(**.fme 22例**。各カードは .fme → 中間 .egi → 生成 .fmr の3段表示。移行は .fmr 比較(バイト一致 or 整形差のみ)+全チェックで検証。Yee `.fme` は総合ギャラリー側に掲載) |
...
- **格子 policy**: field は任意の offset vector ではなく
  ...
  「変位 + target offset − source offset」で配列 offset を解決する
  (`yeeRef` / `dYee`)。generated Egison は policy と component indices を渡し、
  `[0,1/2,...]` をコンパイラで直書きしない。
...
- **計量と `lb` の現在の境界**: `embedding` からの induced metric、直交計量と逆計量、
  体積要素、Hodge coefficient、および Laplace--Beltrami の flux と
  flux-divergence 合成は `lib/formurae-geometry.egi` の
...
- **テンソル**: `Vector MathValue` 等の型注釈でテンソルごと受け取り(λ⊗ のスカラー/テンソルパラメタ)、
  ...
  その反転を半セル配置 bit として使う。
  添字つき field は `field v~i @ primal` や `field σ{~i~j} @ primal` のように宣言し、
  ...
  反対称 rank-2 field は `field A[_i_j] @ primal` と宣言し、storage は独立な
...
1. **境界条件**: fork は軸ごとの `periodic` / `mirror` / `fixed value` を yaml で扱う。
   空間依存・field別の境界演算子は未実装である。
2. **大域リダクション**: `sum` / `min` / `max` / `absmax` は診断出力に使えるが、
   reduction 結果を同じ step の式や動的 CFL `dt` へ戻す表層機能は未実装である。
3. 共有プリンタは多項式・有理式・格子参照・記号・宣言済み scalar extern 関数を扱い、
   CAS initializer `:=` も使える。任意の低水準 Formura/C 式は raw initializer/helper として残す。
4. **一般次元の残課題**: `.fme` 生成経路の scalar/vector/rank-2/k-form は
...
- field別・空間依存の境界演算子と、reduction を時間積分へフィードバックする機能
