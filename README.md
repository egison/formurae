...
3. **限界安定 dt × temporal blocking × 袖幅2**: Maxwell を dt = 0.4dx(中立安定域)で
   TB4 と組み合わせた場合のみ発散を観測。TB なしでは有界。Formura 2.3.2 の TB 実装か
   同梱 MPI スタブとの相互作用が原因候補(実 MPI での切り分けが未了)。
   Yee スキームは袖幅1なので実用上は回避可能。
