...
Formurae の collocated Maxwell。`mode` が離散化と標準 prelude を選び、
`curl` は `epsilon`・添字微分・明示縮約から定義された通常の Formurae 演算子として
自動ロードされる:
...
| `examples/elastic3d/` | **弾性波(Virieux)**(.fme の **添字記法2行**: `field v~i @ staggered`、`field σ{~i~j} @ staggered` と宣言し、`v'~i = v~i + (dt/ρ0) * contractWith (+) (∂_j σ~i~j)`、`σ'~i~j = … λ * g~i~j * contractWith (+) (∂_k v'~k) … g~i~k . ∂_k v'~j …` と書く。繰り返し上下添字は `contractWith` または `.` で明示的に縮約し、上げ下げは metric を明示する。`@ staggered` 宣言で ∂_a が対象配置アンカーの半セル差分に = Virieux 格子を導出。P/S 両波速を1回で実測) |
...
| `gallery/dsl/` | **Formurae ギャラリー**(**.fme 22例**。各カードは .fme → 中間 .egi → 生成 .fmr の3段表示。移行は .fmr 比較(バイト一致 or 整形差のみ)+全チェックで検証) |
