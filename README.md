...
| `gallery/` | **全応用例の可視化ギャラリー**(`gen.sh` → `tools/render.py` → `index.html`。外部ライブラリ不要 — PNG/SVG を標準ライブラリだけで生成。ブラウザで `gallery/index.html` を開く。各カードの「Egison ソース全文」は `tools/embed_src.py` が .egi から再埋め込み) |
