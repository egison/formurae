...
```
.fe      : 表層 DSL(field 宣言+添字記法・微分形式の方程式)     ← v1: 5例が移行済
...
.fe の Maxwell(全28行のうち物理部分)— 方程式は**添字も成分もないベクトル方程式**:
...
cabal build       # .fe コンパイラ fec をビルド(base のみ; make が cabal run 経由で使う)
...
| パス | 内容 |
|---|---|
| `fec/` + `fec.cabal` | **.fe コンパイラ(DSL v1)**: 表層 DSL(`field E : vector`・`E' = E + dt * curl B`・`B' = B - dt * d E'`)を埋め込み形 .egi に変換。Haskell(base のみ)、リポジトリ直下で `cabal build` / `cabal run -v0 fec -- model.fe`。意味論は Egison 側に一本化した薄い変換層 |
...
| `gallery/dsl/` | **DSL ギャラリー**(DSL で書かれた7例: **.fe 5例**(Maxwell・DEC・拡散・KG・KS、カードは .fe → 中間 .egi → .fmr の3段表示)+ v0 埋め込み2例(弾性波・計量トーラス)。移行は .fmr 比較で意味保存を検証) |
| [`DSL-DESIGN.md`](DSL-DESIGN.md) | **DSL 設計メモ**(v0 = fmrdsl 埋め込み層は実装済・maxwell3d で実証; v1 = 添字記法+微分形式の表層構文案とロードマップ) |
...
- **Maxwell 微分形式版(DEC)**: E を 1-form、B を 2-form として d と ⋆d⋆ で記述。
  配置情報は一切書かず、形式の次数から Yee 格子が出る。**生成 .fmr は手書きスタガー版と
  バイト一致**、生成は d(dE')=0 の CAS 検査に合格した場合のみ実行される。
