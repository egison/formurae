...
CC=${CC:-cc}
FIXTURE="$ROOT/tests/fixtures/pre_algebraic.fme"
TMPDIR_ROOT=${TMPDIR:-/tmp}
WORK=$(mktemp -d "$TMPDIR_ROOT/formurae-pre-pipeline.XXXXXX")
...
cd "$ROOT"
cabal run -v0 -j1 formurae-pre -- "$FIXTURE" > "$WORK/model.egi"
...
if cabal run -v0 -j1 formurae-post -- "$WORK/tampered.feir" \
     > "$WORK/tampered.fmr" 2> "$WORK/tampered.err"; then
  printf 'formurae-post accepted a tampered logical registry fingerprint\n' >&2
  exit 1
fi
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_scalar_delta_1d.fme" \
  > "$WORK/derivative.egi"
...
cabal run -v0 -j1 formurae-post -- "$WORK/derivative.feir" > "$WORK/derivative.fmr"
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_kronecker_delta_hygiene.fme" \
  > "$WORK/kronecker.egi"
...
cabal run -v0 -j1 formurae-post -- "$WORK/kronecker.feir" \
  > "$WORK/kronecker.fmr"
...
# syntax across multiple indented lines.  formurae-pre preserves the layout needed
# by match clauses, while Egison remains responsible for normalization.
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_raw_operator.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/raw-operator.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/formurae_standard_ops.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/standard-ops.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/formurae_native_rank2_ops.fme" \
  > "$WORK/rank2-ops.egi"
...
# formurae-post alone resolves sampling placement.
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_conditional.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/conditional.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_coordinate_targets.fme" \
  > "$WORK/coordinate-targets.egi"
run_machine "$WORK/coordinate-targets.egi" "$WORK/coordinate-targets.feir"
cabal run -v0 -j1 formurae-post -- "$WORK/coordinate-targets.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_tensor_locals.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/tensor-locals.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_symmetric_local_mismatch.fme" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_antisymmetric_update_mismatch.fme" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_conservative_local.fme" \
...
cabal exec -v0 runghc -- -isrc tests/pre_conservative_local_feir.hs \
...
cabal run -v0 -j1 formurae-post -- "$WORK/conservative-local.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_local_forward_reference.fme" \
  > "$WORK/local-forward.egi"
run_machine "$WORK/local-forward.egi" "$WORK/local-forward.feir"
if cabal run -v0 -j1 formurae-post -- "$WORK/local-forward.feir" \
     > "$WORK/local-forward.fmr" 2> "$WORK/local-forward.err"; then
  printf 'formurae-post accepted a forward reference to a step-local field\n' >&2
  exit 1
fi
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_local_placement_mismatch.fme" \
  > "$WORK/local-placement.egi"
run_machine "$WORK/local-placement.egi" "$WORK/local-placement.feir"
if cabal run -v0 -j1 formurae-post -- "$WORK/local-placement.feir" \
     > "$WORK/local-placement.fmr" 2> "$WORK/local-placement.err"; then
  printf 'formurae-post implicitly resampled a local materialization\n' >&2
  exit 1
fi
grep -F 'grid placement mismatch' "$WORK/local-placement.err" >/dev/null

cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_invalid_predicate.fme" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_select_placement_mismatch.fme" \
  > "$WORK/select-placement.egi"
run_machine "$WORK/select-placement.egi" "$WORK/select-placement.feir"
if cabal run -v0 -j1 formurae-post -- "$WORK/select-placement.feir" \
     > "$WORK/select-placement.fmr" 2> "$WORK/select-placement.err"; then
  printf 'formurae-post accepted Select branches at incompatible placements\n' >&2
  exit 1
fi
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/formurae_runtime_tensor_ops.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/runtime-tensor-ops.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/formurae_metric_tensor_ops.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/metric-tensor-ops.feir" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/musical-variable.feir" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/hodge-variable.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_dot_shadow.fme" \
  > "$WORK/dot-shadow.egi"
...
cabal run -v0 -j1 formurae-post -- "$WORK/dot-shadow.feir" > "$WORK/dot-shadow.fmr"
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_wide.fme" \
  > "$WORK/wide.egi"
...
cabal run -v0 -j1 formurae-post -- "$WORK/wide.feir" > "$WORK/wide.fmr"
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/ks3d/ks3d.fme" \
  > "$WORK/grid-whole.egi"
...
cabal exec -- runghc -isrc tests/pre_grid_whole_feir.hs \
...
# applies the equation target's lower index at each call site; formurae-pre does
# not infer result variance from the operator name or body.
cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/maxwell3d/maxwell3d.fme" \
  > "$WORK/maxwell.egi"
...
cabal run -v0 -j1 formurae-post -- "$WORK/maxwell.feir" > "$WORK/maxwell.fmr"
grep -F "E_down1'[i,j,k] = E_down1[i,j,k]" "$WORK/maxwell.fmr" >/dev/null

cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/maxwell3d_yee/maxwell3d_yee.fme" \
  > "$WORK/maxwell-yee.egi"
run_machine "$WORK/maxwell-yee.egi" "$WORK/maxwell-yee.feir"
cabal run -v0 -j1 formurae-post -- "$WORK/maxwell-yee.feir" > "$WORK/maxwell-yee.fmr"
grep -F '(B_down2[i,j,k] + (-1) * B_down2[i,j,k-1]) / dz' \
  "$WORK/maxwell-yee.fmr" >/dev/null

cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/diffusion3d/diffusion3d.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/indexed-derivative.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/elastic3d/elastic3d.fme" \
  > "$WORK/elastic.egi"
grep -F '(FormuraeInternalDiff sigma~i~j)..._j' \
  "$WORK/elastic.egi" >/dev/null
run_machine "$WORK/elastic.egi" "$WORK/elastic.feir"
cabal run -v0 -j1 formurae-post -- "$WORK/elastic.feir" > "$WORK/elastic.fmr"
...
# it to second jets, and formurae-post selects the compact five-point derivative.
cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/highorder4/highorder4.fme" \
  > "$WORK/highorder.egi"
grep -F 'def FormuraeInternalScalarDelta u := Formurae.scalarLaplacian' \
...
cabal run -v0 -j1 formurae-post -- "$WORK/highorder.feir" > "$WORK/highorder.fmr"
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/tests/fixtures/pre_metric_forms.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/metric-forms.feir" > "$WORK/metric-forms.fmr"
grep -F "H_1'[i,j] = (-1) * dx * i * A_2[i,j] + (-1) * A_2[i,j]" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_embedded_volume.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/embedded-volume.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_embedded_ambient_metric.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/embedded-ambient-metric.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_remaining_primitives.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/surface-cutover.feir" \
...
cabal run -v0 -j1 formurae-pre -- \
  "$ROOT/tests/fixtures/pre_variable_scalar_delta_equivalence.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/scalar-delta-equivalence.feir" \
...
cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/maxwell_dec/maxwell_dec.fme" \
...
cabal run -v0 -j1 formurae-post -- "$WORK/maxwell-dec.feir" > "$WORK/maxwell-dec.fmr"
...
while read -r geometry check_source; do
  cabal run -v0 -j1 formurae-pre -- "$ROOT/examples/$geometry/$geometry.fme" \
    > "$WORK/$geometry.egi"
  ...
  cabal run -v0 -j1 formurae-post -- "$WORK/$geometry.feir" > "$WORK/$geometry.fmr"
...
printf 'formurae-pre algebraic and derivative pipeline tests: ok\n'
