...
# truncated definition body cannot pass on a merely nonempty formurae-pre output.
compile_pipeline \
  tests/fixtures/pre_reserved_string_near_miss.fme \
...
compile_pipeline tests/fixtures/pre_index_completion.fme index-completion
cabal exec -v0 runghc -- -isrc tests/pre_index_completion_feir.hs \
...
up_error=tests/fixtures/pre_index_completion_up_error.fme
cabal run -v0 -j1 formurae-pre -- "$up_error" > "$WORK/index-completion-up.egi"
if "$ROOT/tools/run_formurae_normalization.sh" "$EGISON_DIR" \
...
anonymous_error=tests/fixtures/pre_anonymous_index_nonunification.fme
cabal run -v0 -j1 formurae-pre -- "$anonymous_error" \
...
fixed_error=tests/fixtures/pre_fixed_parameter_error.fme
cabal run -v0 -j1 formurae-pre -- "$fixed_error" > "$WORK/fixed-parameter.egi"
if "$ROOT/tools/run_formurae_normalization.sh" "$EGISON_DIR" \
...
printf 'formurae-pre user definition and higher-order pipeline tests: ok\n'
