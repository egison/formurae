...
compile_pipeline tests/fixtures/pre_fec_macro_torus.fme macro-torus
compile_pipeline tests/fixtures/pre_fec_macro_torus_direct.fme macro-torus-direct
diff "$WORK/macro-torus.fmr" "$WORK/macro-torus-direct.fmr" >/dev/null

# Each instantiation freshens its lifted locals.
cabal run -v0 -j1 pre-fec -- tests/fixtures/pre_fec_macro_hygiene.fme \
  > "$WORK/macro-hygiene.egi"
grep -F 'def q :=' "$WORK/macro-hygiene.egi" >/dev/null
grep -F 'def q2 :=' "$WORK/macro-hygiene.egi" >/dev/null

printf 'pre-fec macro expansion tests: ok\n'
