...
1. **ユーザ定義テンソル演算子の表層化**
   - `def curl X~i = epsilon~i~j~k . d_j X_k`
   - `def div X = d_i X~i`
   - `def grad u_i = d_i u`
   - `def stress_i_j v = ...`
...
2. **スカラー関数・テンソル関数の区別を型/種として持つ**
   Egison の型は Scalar/Tensor の区別を中心にし、テンソルの階数までは型で
   管理しない。この方針に合わせ、Formurae でも演算子定義の基礎は
   「スカラー関数か、添字を受け取るテンソル関数か」の区別に置く。
   これに合わせて、積の記法も Egison と同じく `*` はスカラー関数同士の
   掛け算に限定し、テンソル同士の積・添字縮約を伴う積は `.` で書く。
   例えば `epsilon~i~j~k . d_j X_k` のように、添字を持つ量の積は
   テンソル演算として扱い、スカラー係数との積だけを `*` に残す。
...
3. **`fec` の特殊規則を減らし、演算子を通常定義へ寄せる**
   ...
   特別扱いを持つ。`Δ` をプレリュード定義へ格下げした方針を進め、`curl`・`divg`・
   `grad`・高階差分なども、可能な限り `.fe`/Egison 側の通常の演算子定義として
   表現する。`fec` は「定義を解釈する」のではなく「表層定義を Egison 埋め込み形へ
   運ぶ」薄い層に保つ。

   目標となる意味論の核は、かなり小さくできるはず:
   - `+`: スカラー関数/テンソル関数の加法
   - `*`: スカラー関数同士、またはスカラー関数による係数倍
   ...
   Hodge 因子などは、演算子プリミティブではなくテンソル値/係数場として扱う。
   これにより、Formurae の言語核は「スカラー積・テンソル縮約・座標微分」だけになり、
   数学演算子の多様性は Egison 側のユーザ定義で表せる。
