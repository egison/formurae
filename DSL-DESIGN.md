...
などの AST へ構文木化するようにした。parser error は式全体と失敗近傍を表示する
...
3. **`use` は生成 Egison 定義を選ぶ宣言にする**
   `use vector-calculus { curl, divg }` は、Haskell 側で `curl` や `divg` を特別展開する合図ではない。
   現在の `dimension`/`axes` から `dGrad`、`curl`、`divg` の Egison 定義を生成 `.egi` に出す合図である。
   `fec` は vector equation を component lowering し、テンソル式そのものは Egison 側の定義へ渡す。
...
5. **残課題**
   ...
   parser error は式全体と失敗近傍を返すようになったため、次は source span つき診断と
