...
**v2.0(2026-07-11): formurae-pre / Egison / FEIR / formurae-postへのcutover** —
現在の責務境界は、以下のpipelineと `src/Formurae/Pre/`、`src/Formurae/FEIR/`、
`src/Formurae/Post/` の実装、`spec/feir-primitives.sexp`、および検証testで規定する。
以下のv1.36以前の節は設計履歴として残すが、旧`fec`、native marker、generated derivative callback、
Egison-side FMR printerを現在の実装説明として読まない。

```text
.fme -> formurae-pre -> .egi -> Egison -> .feir -> formurae-post -> .fmr -> Formura -> C
```

`formurae-pre`はfrontendとsource map、Egisonはtensor/index algebra・pure user function・analytic
differentiation、`formurae-post`はplacement・exact stencil・auxiliary lifetime・storageを担当する。
...
`let`、lambda、`match`、`withSymbols`、`generateTensor`の意味はEgisonが持ち、formurae-preは
...
user `def`のfunction headには結果添字を書かず、formurae-preはfunction名またはbodyからresult varianceを
...
formurae-preはuser `def`本体の式構造や計算履歴を分類してresult contractを付けず、1行の式も
...
`def`(式=Egison が値に評価)と対になる **`macro`(文列=formurae-pre が展開)** を導入した。
...
formurae-post への直書きで代替したもの」という位置づけが実装で裏づけられ、
...
formurae-post が **field-jet を含まない materialize を凍結**する: per-basis の
...
(「可変計量の δ は formurae-post が補助 field を計画」等の要求時代の記述)も
...
(radius 属性の解釈だけを formurae-post で精密化)。チェーン(ordered)各段と
...
(sexp+生成器・operators/feir.egi・Pre の parse/effect/kind/emit・formurae-post)。
formurae-post は境界行を **index guard の FSelect**(`if i == 0 …
...
導出されるので、formurae-post は無変更で `sbpd_x q_x` の閉包選択・配置検査・
...
旧 sbpd 版とバイト一致**(formurae-post の normalizeAdd が構造的全ソートなので
...
**v2.21(2026-07-19): k ≥ 2 の SBP 閉包構成器 — 「幅 × 境界」の積が全域で定義される(Phase B)** —
staggered SBP 対の一般構成器を実装し、sbp 軸の明示エラーだった幅を
閉包つき降下に開放した。**数学**: Q⁺ = H_d D⁺・Q⁻ = H_p D⁻ を未知数に
取ると、境界行の精度条件(Σ Q x̂^p = H·p x^{p−1}、境界次数 s = k)・
SBP 恒等式(Q⁺ + (Q⁻)ᵀ = −d₀e₀ᵀ、片端+鏡映)・外挿精度が全て
**(Q, H, d) に対して affine** になり、既存の厳密 RREF 一発で可解性判定と
正準解(自由変数 = 0、列順 = ノルム→外挿→近サンプル優先なので遠い係数
から消える)が同時に得られる。構造(行数・支持幅・外挿幅)は小さい方
から探索し、正ノルム+有限 N 完全検証を通る最初の候補を採る。k = 1 の
既存閉形式が同じ定式化の解であることは手計算で照合済み(d₀ = (3/2, −1/2)
が恒等式の (0,0)/(1,0) 成分から再現)。interior と一致する冗長閉包行は
刈る(free-zero の副産物、guard の無駄を防ぐ)。2 階閉包は解かずに
**厳密積 D⁻D⁺ の境界行**として読み取る(検証器の合成一致検査と定義が
一致)。実測: **k = 2, 3 が成立**(k=2: D⁻ 3 行 {0,1,3}・D⁺ 3 行・
H_p = [7/18, 9/8, 1, 71/72]・H_d = [13/12, 7/8, 25/24]・d₀ =
(15/8, −5/4, 3/8) = 古典 3 点 2 次外挿)。**言語側**: レコードに
`sbpPrimalToDualLow/High` を追加(k ≥ 2 では D⁺ も閉包が要る; dual 配置
ターゲットの high guard は total_grid − 2 起点 = 域外 half slot は決して
guard しない)、`lowerSbpGuardedDerivative` が pair 数を取り、wide 1 階
全半径+profile 全 accuracy の 1/2 階が閉包化(2 階 interior の固定
[1,−2,1] は合成由来に置換 = k=1 バイト不変)。compact な幅広 2 階
`∂'^2`(maximal-accuracy 5 点)は**合成と別物で SBP 分解が無い**ため
意図的にエラーのまま(Δ/profile 経由の合成 2 階が閉包つきの道)。3 階
以上も同様。**E2E**: 新例題 sbp_highorder4(∂' 両方向の flux 形、
SAT = (18/7)τκ/dx²、dt = 0.0125 dx² で Euler 時間誤差を空間誤差の下へ):
モード誤差 4.96e-06(N=64, t=100)・エネルギー単調・壁 2.7e-08。
**時間精緻化で空間誤差を分離した実測収束: N=64 → 128 で 5.07e-06 →
6.22e-07 = 比 8.15 = 3.03 次**(境界 s=2 の max ノルム定石 s+1 どおり、
完了条件「3 次以上」達成; dt ∝ dx² のままだと Euler の O(dt) が
1.2e-05 で支配して見かけ 2 次に落ちる点に注意)。ワイヤ・manifest 不変
(閉包は formurae-post 内部)なので既存例題は .egi/.feir/.fmr 全て完全バイト
不変。suite 全緑+make all 31 例全緑。残: Neumann/特性 SAT の定型化
(Phase C)・compact 2 階の Mattsson–Nordström 型 D2 閉包(採るなら別軸)。

**v2.22(2026-07-19): SAT の定型化 — 宣言が定数を供給し、境界外挿と
ペナルティ・マクロで Neumann・特性境界が閉じる(Phase C)** —
boundary-declaration の原則 3(「SAT は方程式の内容。宣言は H⁻¹ 端重み等を
名前つき定数として式へ供給し、定型は surface macro で展開」)を実装した。
**(1) 宣言供給の定数**: `boundary a : sbp` が壁しきい値 `sbpLoA`(0.5·d_a)/
`sbpHiA`((total_grid−1.5)·d_a)と逆境界ノルム `sbpHinvA`(= 1/(h_p,0 d_a)、
最小対 2/d_a)+モデルが綴りうる幅ぶんの `sbpHinv<2k>A`(pd 半径と
staggered profile accuracy を走査; k=2 は 18/(7d_a))を**通常パラメータとして
先頭に注入**(exact 有理値の raw 文字列・ユーザ param から参照可・名前衝突は
fatal)。命名は camel 合成 — **underscore は Egison CAS で添字に読まれる**
ため `sbpLo_x` は不可(実測でパース失敗を確認して確定した罠)。
**(2) 境界外挿 opaque `boundary.sbp-trace`**(表層 `sbpx_a e`): 壁行で
d₀ᵀ/d_Nᵀ による境界値・内部 0 の index-guarded 式。surface マクロは
格納シフトを書けないため、外挿は opaque が必須(TODO の「最小」案どおり)。
grid-whole の写経で全層貫通(manifest 追加=新ハッシュ 0ee43246…・
operators/feir.egi・Index/Effect/TypeCheck/Emit・PrimitiveContract・
formurae-post 降下+推論)。ゲート: sbp 宣言軸限定・dual 配置 operand 限定・
最小対のみ(SbpTraceRequiresSbpAxis 等の静的エラー)。
**(3) SAT prelude マクロ**(sbp 軸ごとに生成): `satDirichlet_a(u,g,coef)` =
両壁の減衰項、`satNeumann_a(flux,glo,ghi)` = `sbpHinvA*(sbpx_a flux − g)` の
流束置換。マクロ機構の罠 2 件を修正 — 呼び出し照合が `TEIdent name []`
限定で `_a` 付き名が素通り(フルスペル照合へ)/**locals を持たないマクロは
展開結果が捨てられていた**(マクロ名使用検出で書き換え判定へ)。特性 SAT は
向き・インピーダンス依存の方程式内容なのでマクロ化せず表層で書く。
**例題**: sbp_neumann(断熱壁 cos モード、`satNeumann_x(q,0,0)` の厳密
流束置換で **熱量 1ᵀH_p u のドリフト 3.4e-15**・エネルギー単調・実測
2.04 次 = 2.81e-05→6.84e-06)と sbp_wave_open(特性 SAT
`sbpHinvX*(p ± sbpx_x v')`、エネルギー法で dE/dt = −2p₀²−2p_N² を導出、
**E は初期値を一度も超えず**最終 1.7e-4·E0・mid-flight 実測 1.98 次 =
4.85e-02→1.23e-02・反射残差 1.4e-02→6.1e-03; v は leapfrog 整合の
f(x+dt/2) で初期化、x0/w は v2.19 規準どおり物理値)。既存 sbp 例題 4 本を
定数+マクロのイディオムへ書き換え(driver 測定値は全て書き換え前と同値)。
...
**v2.24(2026-09-07): 曲線座標の弾性波と同一点配置のSBP差分** —
円筒・球座標の三次元速度・応力系を追加した．両者は同じ `strain`，
`stressRate`，`stressDiv` の通常の関数定義を共有する．解析的な `∂/∂` は
計量だけを微分し，添字つき `∂` は計量と応力の流束全体を差分化する．
体積係数と計量を参照先の点で評価してから流束の差分を取ることが，
速度のひずみとの離散的な部分積分関係を保つ．`local` 配列に中間値を置き，
球座標の式の一括展開を避ける．弾性波専用のコンパイラ規則は追加しない．

`boundary ... : sbp` の下で，同一点配置の1階の式全体の微分要求に，
内部2次・境界1次の対角重みSBP差分（D2-1）を追加した．
内部は中心差分，左端は前進差分，右端は後退差分を使い，領域外を参照しない．
同軸の外側ステンシルからずらして評価する要求は従来同様に拒否する．
高次・幅指定のSBP差分族は既存のスタガード構成器を使い，今回追加した
同一点配置の行とは区別する．

数値検証は解析的なねじれ振動（境界を含む），デカルト座標の厳密解の
座標変換（全方向・全成分の内部誤差），時間収束，独立C参照との照合，
全成分を励起する1万ステップのエネルギー検査からなる．速度Verlet法に
対応する修正エネルギーと通常のエネルギーを別々に測る．再現用スクリプトは
現行Formuraeに対応するEgisonの `1a0298c67cc4` を別ディレクトリへ展開する．
2026-09-07のEgison HEADでの型検査変更への移行は既存例にも影響するため，
今回の数値結果の依存版を明示して固定した．詳細は
`examples/elastic_curvilinear/README.md` と同所の結果JSONに記録する．

**v2.25(2026-09-08): 利用者定義の添字演算子の実行比較** —
添字に応じた成分の対応付けを行う `tensorMap2` と，成分を指定の関数で
まとめる `contractWith` を高階の利用者関数で組み合わせる例を追加した．
局所添字を使う内積と対称部分，関数として渡す回転，次数を入力から求める
外微分も，`examples/elastic_curvilinear/comparison/expressivity/` に収録する．
UFL・SymPy・NumPy・Devitoの対応する記述と，NRPyLaTeX・OpenSBLIの
置換による定義を実行し，値，必要な添字操作，名前の衝突を比較する．

この実験で，解析的な `∂/∂` を関数値として渡す場合に，前処理で変換した
内部名の定義が生成されない不備を修正した．直接呼出しの変換に加え，
関数値として使う場合はEgisonの `∂/∂` を参照する定義を生成する．
外微分の式自体をコンパイラに登録する必要はない．

さらに，記号計算で更新式全体が0などの定数になった場合にも，
格子上の配列としてFormuraへ渡せるよう，更新関数の出力に `double []` を
明示する．初期化で指定していた配列型を更新にも保つ変更である．
2次元・3次元の外微分の合成が0へ整理される例を含め，生成Cを実行する．

**v2.27(2026-09-10): Formura の標準コード生成経路に統一** —
`init`・`step`・`local` を中心とする記述を，Egison 正規化，FEIR 検証，
Formura コード生成，Formura 本体による C 生成へ通す構成に統一する．
v2.26 で追加した `stage`・`procedure`・`runtime`・`prepare`・`advance`・`observe`
による独立した実行経路と，それに依存するデモ・生成物・実行スクリプトを削除した．
MPI による領域分割や時間方向のブロッキングを使わない専用経路を，
Formura の通常の実行経路と並行して維持しない．
テンソルの全軸を整数で選択した式をスカラーとして検査する修正と，
既存のテンソル演算・曲線座標・局所中間配列の機能および検証は維持する．
今後のデモでは，モデルの記述とともに Formura の通常経路での実行と，
適用する並列化・ブロッキングの条件を確認する．
対象は Formura 本体を拡張せず既存の機能で実装できるシミュレーションに絞り，
初期条件・境界処理・補助計算を含めて題材の選定時に確認する．

**v2.29(2026-09-11): 式への添字適用と成分宣言の計量** —
括弧でくくった式の直後に添字を書く `(f x)~i_j` を，構造化された式文法の要素として受理する．
従来は def 本体が構造化文法に失敗したときだけ生の Egison 式として通っていた綴りで，
init・step では拒否され，def 本体でも `∂` などの正準演算子と同居できなかった．
新しい規則は既存の `(f x)...~i_j` と同じ添字付加ノードを作り，型検査・効果解析・Egison 出力を
共有する．数値だけの添字列(`_1_1`)も受理し，`.` による縮約の分割は添字付加の綴りがあっても行う．
併せて，計量成分など周辺テンソルの数値添字成分(`g_1_1`)をスカラーとして型付けする．

`metric tensor [[...], [...]]` と `metric volume EXPR` を追加した．計量を成分で宣言し，
Egison が対称性を検査して逆行列(次元 1〜3 の閉じた式)と体積要素 sqrt(det g) を導く．
FEIR には `general-metric` の幾何種別を加え，直交性検査済みの旗を立てずに検証を通す．
この幾何では canonical `Δ`・`δ`・`hodge` を出力時にエラーにし，流束形を明示的に書かせる．
用途は `examples/excitable_torus/excitable_torus_twisted.fme`(トーラスのねじった座標
φ = ψ + θ；両方向とも周期で計量は非対角)で，直交座標の結果と 2 次精度で一致する．

**v2.30(2026-09-11): backquote 原子の FEIR 通過** —
`` `(x - cx) `` のように backquote でくくった式は，Egison の正規化では原子として扱われ（展開されない），
FEIR の符号化では中身の式をそのまま符号化する．従来は `quote is not part of FEIR v1` で拒否していたが，
quote は CAS への指示であって生成コードには関係がない．座標変換の写像を `∂/∂` で微分して物質テンソルを
導く変換光学の例で，中心をずらした座標を原子にしないと正規化が数分かかる（原子にすれば 1 分強）．
同じ例で，二つの装置の写像を一つの式に足すと共通分母の合成で項が掛け合わさり正規化が 10 分になるため，
装置ごとに別の場へ凍結して step で旗により選ぶ．

**v2.31(2026-09-13): 反復回数を固定した射影法の例 `taylor_couette`** —
軸対称テイラー・クエット流れ（`examples/taylor_couette`）を追加した．圧力補正型の射影法で，
圧力補正のポアソン方程式を回数固定（4 回）の重み付きヤコビ反復として `step` に展開する．
`TODO/nested-stencil-iteration.md` の工程 0（Formura を変えない基準版）で，入れ子の反復構文と
比較する基準になる．言語機能の追加はない．記述の要点と判明した制約は次のとおり．

- スタッガード配置は `field u_i @ dual`（成分 k は軸 k 以外で半整数点，すなわち面）と
  `scalar @ dual`（セル中心），`scalar @ primal`（セル角）で表し，配置の違う量の積は
  `resample(値, ビット...)` で明示的に補間する（ビット 1 が半整数点）．
  面に置く一時値はスカラーの `local` では宣言できないので，`local q_i @ dual = [| ..., ... |]_i` の
  成分として持つ．
- 無印の `∂_a e` は式全体を標本化して差分する半セル差分であり，係数を挟む流束形
  `∂_r (r ∂_r u)` は一つの式では書けない（連鎖の本体は微分を含まない純粋な式に限る）．
  流束を `local` として実体化し一度だけ差分する．`macro` で `local` を含む演算子
  （円筒座標のラプラシアン）を定義し，呼び出しごとに衛生的に展開した．
- `def` 本体では `dr`・`dz` が Egison 側で未束縛になるので，格子幅を含む定数は `param` に置いて
  参照する．`_` を含む名前は添字つきの名前として解釈され，`m`・`d`・`e` などの一文字名は
  生成コードの予約名と衝突する．
- 壁のある軸でも，生成コードはゴースト格子点の座標を格子数で割った余りから求めるため，
  ゴースト格子点で読んだ座標に依存する式（壁のマスクや壁速度）は誤る．壁面をすべて格納される
  格子点に置き（`length_per_node = (R2 − R1) N/(N − 1)`），外壁の外側の一点を更新式で 0 に
  固定して，YAML の `fixed 0.0` による内壁のゴーストと同じ扱いにした．
- Formura の C 生成時間は，`step` の中の依存関係の深さ（展開した反復回数）に対して指数的に
  増える（1，2，4 回で 0.3 秒，1.7 秒，83 秒，8 回は 15 分で打ち切り）．参照半径 `Ns` は 4 回で 6．
  多数回の反復を展開する書き方は成立せず，反復回数を実行時に決める構文が必要である．

検証（`make taylor_couette`）：射影の恒等式 `div_h u' = dt (rhs − L_h φ)` が 7×10⁻³² で成立，
定常の周方向速度が層流クエット解に最大 3×10⁻⁴ で一致，圧力の遠心力とのつり合いが範囲比
9×10⁻⁴，定常状態の発散と子午面速度は 10⁻¹⁷ 程度．
デモ（`make taylor_couette-demo`，`run.py`・`driver.c`・`render.py`）：Re = 100，65×256 格子，
z 方向 4 ランクで層流クエット解から出発し，擾乱が成長率 0.118 で育って隙間幅の 2 倍の波長の
テイラー渦 2 対に飽和する．粗い格子で Re = 60 は減衰し Re = 80 は成長して既知の閾値 Re ≈ 68 を
挟む．ギャラリー（`html/{en,ja}/gallery.html`）の流体の節にカードを追加した．

**v2.32(2026-09-14): 引数なしの値定義** —
`def NAME = BODY` を受理し，名前だけで参照できる値を定義する．引数つきの定義と同じ
Egison 出力経路を使い，スカラー・テンソル・関数の値と複数行の本体を扱う．
定義の参照順序，離散演算を含む式への制約，エラー時の参照元表示は既存の仕組みを共有する．
`transformation_optics.fme` の値定義から `unused` と呼び出し時の `0` を除去した．
同じ変更を全例題と検証用の `.fme.inc` にも適用した．初期条件を差し替える実行スクリプト，
テスト用の計量定義，README と日英 gallery の掲載コードも引数なしの定義へ揃えた．

**v2.33(2026-09-14): 時間変化しない場の宣言** —
`static field G1{_j_k} @ collocated := gammaTheta` は、宣言した格子配置で
初期化時に一度だけ値を計算し、以後は保持する場を定義する。通常の `field` と
同じ型・添字・配置の指定を使い、利用者による再初期化・更新を禁止する。
右辺は座標・パラメータ・計量・先に初期化された固定場に依存できる。
Egison の評価後に FEIR の `static-state` として依存を検査するため、`def` を
経由した通常の場や次時刻の値への依存も検出する。自己参照・前方参照・循環参照、
初期化の欠落・重複、解析できない raw 初期化も拒否する。
固定場の宣言は通常の場と同様に登録情報のハッシュに含める。
formurae-post が通常の Formura 配列と恒等更新を生成し、Formura 本体の拡張は行わない。
液晶・変換光学・興奮波と精度検証用の追加ソースにある固定場をこの構文へ移す。

**v2.34(2026-09-17): 添字記号ごとの成分数** —
`index a, b : 9` で空間次元と独立した添字の範囲を宣言する。例えば2次元で
`f_a` は9成分，`c_a_i` は9×2成分，`M_a_b` は9×9成分となる。
未宣言の添字は空間方向を表し，その範囲は `dimension` に従う。
成分の範囲は1から宣言した正の整数までで，場の宣言時に各軸のサイズを固定する。
異なるサイズや空間方向・成分番号の区別を変える名前の付け替えを拒否する。
対称・反対称な場は，同じサイズかつ同じ種類の2軸を持つ。
`static field`，添字付き `local` / `let`，関数引数，縮約，数値添字にも適用する。
関数内では宣言した添字も局所的な記号として束縛し，呼び出し側の添字と衝突させない。
関数から返る値やテンソルリテラルへの添字適用では，Egisonの評価時にサイズを照合する。

FEIRの通常のテンソル形状は任意の正の成分数を持てる。場の宣言に
`spatial-slots`（空間方向を表す軸の位置，1から数える）を必須で記録し，
格子配置ではその軸だけを数える。空間次元と同じサイズの成分番号も区別する。
微分方向・計量・微分形式は従来の空間方向に限定する。添字を省略した
`local q : tensor` の値にはこの区別がないため，`index` 宣言があるモデルでは
局所場の添字を明示する。通常の添字付き場の階数は従来どおり2以下とする。

成分ごとの通常のFormura配列へ変換し，Formura本体は変更しない。
`pre_index_sizes` の検査は，2次元上の9成分の拡散を生成Cコードで実行し，
離散化した方程式の解析解と照合する。縮約，9×2成分の微分と配置，
3×3の対称・反対称成分，時間方向のブロッキング，誤った添字の拒否も確認する。
過去の `family` 案による成分集合の宣言は，この `index` 宣言で置き換える。

**v2.35(2026-09-17): 局所テンソルの数値成分と波の例の整理** —
明示的に宣言した `local u_i` の `u_1`，`local q_a` の `q_9` などについて，
全軸を数値で指定した成分を型検査でスカラーとして扱う．これまでは永続する `field` の
成分だけがこの判定の対象になり，局所ベクトルの成分を含むスカラー式を誤って拒否していた．
一部の軸だけを選ぶ参照と記号添字を持つ参照は引き続きテンソルとして扱う．

二次元の波では，既存の `def` 内でのEgison `generateTensor` を使い，重みと方向の表から
9成分の初期化・衝突・境界処理をまとめる．テンソル全体に対する条件分岐も利用する．
状態を更新する前に使用を終える密度・速度・占有率は `local` から読めるため，
`savedRho`・`savedU_i`・`savedEps` の4スカラー成分を除去する．
これは例の記述の整理であり，Formura本体や既存の `generateTensor` の仕様は変更しない．
生成式の比較と変更前後の長時間計算の照合は `examples/breaking_wave/verify_refactor.py` で行う．

**v2.36(2026-09-17): 計量と成分番号の併用，座標を変えたD2Q9の実験** —
`index a : 9` のような空間方向とは異なる添字と計量を併用した際，
添字の大きさの検査が裸の `g` を参照していた問題を修正する．計量の束縛は
`g_i_j` と `g~i~j` なので，対応する全成分の参照 `g_#_#` または `g~#~#` を
選んでから形状を検査する．標準名 `metric` と `inverseMetric` にも適用し，
関数の局所引数で同名を使う場合はその束縛を優先する．既存の添字検査へ
非対角計量による上げ下げを追加し，通常版と時間方向のブロッキング版で検査する．

`examples/kinetic_coordinates` では，D2Q9（二次元の九方向の分布）の移動を
微分方程式で解く．計量による内積と体積要素を含む保存形の流束を使い，
座標の写像だけを変えて，同じ移動・衝突・時間積分を実行する．
初期条件・幾何・三段の時間積分・診断値はすべてFMEに記述する．
正規化後は格子サイズごとに通常経路でCを生成し，既存の `extern` から座標係数と
実験の種類を設定する．外部関数は設定値を返すだけで，物理量は計算しない．
新構文やFormura本体の拡張は導入しない．

16×16，32×32，64×64点，三種類の座標，三種類の条件について，一様流保存，
移動の厳密解との誤差，粘性で減衰する流れの座標間の差，質量と物理的な運動量を
比較する．再現手順・測定結果・処理系の版は例題のREADMEと結果JSONへ記録する．
試験写像は平面上の非直交座標を含むが，幾何係数の特別な形を使って一様流を
保存している．一般の写像の離散化，壁・外力・二相流との結合は波のTODOに残す．
...
同例の高精度化には追加構文を設けず，面を挟む差を既存の片側選択でセルへ戻し，
セル内の傾きを制限した直線から面の値を求めるMUSCL法を記述できた．
二段の時間積分SSPRK2は，既存の場に元の状態と段階番号を保存して表す．
最終的な平均の前のEuler候補も含め，非負性と初期値の範囲を検査する．
セル平均の初期値・参照値を四点の数値積分で求め，48条件で総量保存・収束・ぼけを比較した．
64×64セルの最大誤差は，同じ時間積分を使う一次風上法の約6.6〜7.5分の1となった．
初期条件・数値積分・診断値まで通常の生成経路を使い，処理系の変更は行わない．
衝突項・重力・水面との結合は，この輸送試験とは分けて次に検証する．

2026-09-18の `examples/kinetic_viscosity` では，この輸送にBGK衝突項（同じ密度・運動量の
平衡分布へ緩和する項）を加える．九方向の番号と，物理空間の正規直交基底の成分番号を
別の添字で表し，計算座標の定義と独立した内積で平衡分布を記述する．
既存の有限体積輸送と二段の時間更新を使い，構文・処理系は変更しない．
三つの緩和時間でのせん断流は，九方向の連続時間の方程式から導いた減衰率と比較する．
非負性の十分条件には衝突による減少も含め，両Euler候補と平衡分布を検査する．
初期条件・四点積分・参照解・診断もFMEで計算し，50条件の比較と生成コードを保存する．
最も短い緩和時間では32→64セルの最大誤差の収束が基準に届かず，128セルを追加した．
合格基準を維持し，64→128セルで高精度法の全六条件の最大・平均速度誤差の収束を確認した．
重力・壁・水面の検証へ進むための基準例とし，波の動画への結合は別途行う．

2026-09-18の `examples/kinetic_hydrostatic` では，連続時間の外力と水平な反射壁を加える．
静水解 `ρH=exp(−3gY)` からの差を保存・輸送することで，静水を保つwell-balanced法
（定常状態を保つ計算法）を既存の構文で表す．衝突と外力も差の変数で書き換え，
初期状態への上書きや負値の切り詰めは行わない．通常の全分布の輸送を対照とし，
密度の乱れが動くことと，水平外力による総運動量の変化も検査する．
壁では法線方向の物理速度を反転させる．配列値を折り返す `mirror` とは区別し，
上下の補助セルにより両壁の面を記憶領域内に置く．壁の流束は流体側の分布から作る．
格子の写像・物理モデル・数値流束を分け，初期値，四点でセル平均を求める数値積分，時間更新，診断値まで
FMEで記述する．16〜64セルの24条件が通過し，静水の最大流速は0，質量の相対変化は
全条件で最大3.265e-14だった．密度の乱れの投影値の変化は格子細分化で減少した．
水平な滑り壁での検証を基礎に，水面，斜面，壁の精度向上へ進む．

2026-09-18の `examples/kinetic_surface` では，上面を小振幅の水面へ置き換えた．
静水からの変化の一次の項までを残す線形化により，固定した基準領域内のD2Q9の
微分方程式と，水面の高さの時間発展を結合する．外へ出る分布と反対方向の入る分布を
組み合わせて，水面の法線応力（面に垂直な力）と接線応力の条件を課す．
水面の高さには内部の更新と同じ数値流束を使い，基準領域内の密度と水面変位の寄与を
合わせた線形化した総質量を保存する．両方を同じ二段の時間積分で進める．
水面と底面に隣接するセルでは，流体側の隣接セルとの差から面の値を求め，
一定値で近似する場合との比較で境界の精度を調べる．初期条件，幾何，圧力条件，
流束，高さの更新，周期を求める零交差時刻，保存量の診断まで既存構文で記述する．
Formurae・Formura本体の変更は不要だった．物理的な速度方向と方程式を共通に保ち，
直交格子と曲線格子を比較する．参照周期は非粘性・非圧縮の重力波の式から求め，
粘性と圧縮性を含む本モデルの厳密解とは区別する．
16〜96セルの16条件を通過し，総質量の相対変化は最大8.505e-15だった．
64×32の曲線格子では周期差が5%未満の基準に届かず，許容値を維持して96×48を追加した．
最も細かい格子では参照周期との差は最大4.961%，座標間の振幅指標の差は初期振幅の0.00933%だった．
静水と重力0の対照では流速0を保ち，64×32で時間刻みを半分にした振幅指標の差は
初期振幅の最大3.182e-8だった．検証値と生成物を保存し，日英の波のページに動画を掲載する．
水面の条件を水平な基準面で評価する段階であり，任意形状の移動する水面や
水の占有率，巻き込み，水際の移動は次の実装・検証として残る．

2026-09-18の `examples/surface_transport` では，移動する水面の準備として，
水の占有率を指定した非圧縮の流れで運ぶ．占有率はセルを水が占める割合である．
物理空間の流れ関数を面の両端で評価し，その差から面の体積流量を求める．
セルを一周した差が相殺されるため，直交格子と曲線格子のどちらでも流量がつり合う．
水量を保存する流束と，傾きを制限した直線による再構成，二段の時間更新を用い，
時間刻みの十分条件の下で占有率の0〜1の範囲を保つ．更新後の切り詰めや全体補正は行わない．
幾何，初期形状の四点積分，面流量，時間更新，形状の戻りの誤差と保存量をFMEで記述する．
複雑な初期値の式の重複展開は，既存の `static field` へ積分点・重み・値を順に保持して減らす．
これは例題の記述の整理であり，処理系・構文・生成経路の追加は不要だった．
32〜320セルの28条件を通過し，水量の相対変化は最大5.034e-14だった．
128²の戻りの誤差と256²の座標間の差が基準に届かず，精度の基準を保って320²を追加した．
128²での戻りの誤差は，一次風上法の0.2757〜0.3076倍となった．
占有率の上限超過2.605e-12が当初の固定許容値を超えたため，範囲の検査は
更新回数kに応じた max(2e-12, k×機械イプシロン)へ変更し，全記録時刻を再検査した．
これは検査上の許容値であり，浮動小数点誤差の厳密な上界の証明とは区別する．
計算値は補正せず，検証記録に実測の超過量と許容値の式を残す．
D2Q9の速度・密度と圧力条件はまだ結合していない．次は密度と整合する水の質量の輸送，
任意の向きの水面の圧力条件，空気から水へ変わるセルの分布の初期化を検証する．

2026-09-18の `examples/kinetic_transport` では，指定した流れによる占有率輸送を，
連続時間のD2Q9の輸送・BGK衝突と結合する．九つの分布の面流束の和を，密度 `ρ` と
水を表す質量 `m=ρα` の両方の更新で共有する．流束は面を単位時間に通過する量である．
SSPRK2（二つのEuler更新を組み合わせる二次精度の方法）の各段で同じ時刻の流束を使い，
分布と質量を平均した後に `α=m/ρ` を求める．割合そのものを時間方向に平均しない．
四つの再構成した割合の和が `4α` となる性質を用い，面ごとの流出する質量がセルの
質量の1/4以下という十分条件から，水の質量と補集合の非負性を示す．
衝突を含む分布の非負性の条件は別に検査する．切り詰め・全体補正は行わない．
初期条件，写像したセルの幾何，更新，解析解との比較，両Euler候補を含む極値と保存量は
FMEに記述し，既存構文と通常の生成経路だけで実行する．処理系の拡張は不要だった．
物理空間の固定基底の添字 `p` と，計算座標の添字 `i` を分けた衝突式を保つ．
現在は全セルに同じ流体があり，一部に水という印を付ける．密度・速度が変化する中での
印の質量の保存と割合の範囲を検証し，印の外側を空気として扱う自由表面の計算とは区別する．
16²・32²・64²の20条件を通過し，印の質量の相対変化は最大4.885e-15，
一定割合0.37の最大誤差は6.107e-15だった．割合の最大上限超過は2.154e-14で，
固定した検査許容値2e-12以内だった．64²の平行移動の解析解との誤差は，一次風上法の
0.03670〜0.03919倍だった．時間刻み半減と二つの座標の比較も通過し，
密度が初期の範囲から変化したことを保存済みの診断値で追加検査した．
日英の波のページで，割合・流速・密度の保存した場を動画として表示する．
次は任意の向きの水面の大気圧条件，重力，新しく水になるセルの初期化を結合する．
...
通常の実行用スクリプトはユーザーの指定により4Gを標準の上限とし，`EGISON_HEAP_LIMIT` で変更できる．
...
3. ✅ v1: **.fme 表層構文+コンパイラ実装済(2026-07-10)**。まず Python
   ...
   現在は cabal パッケージ `formurae`(ルート formurae.cabal、ソース src/ と
   app/ 以下)で、`cabal build` / `cabal run -v0 formurae-pre --` で使う。
...
6. **計量と Laplace--Beltrami の純粋な公式は Egison にある**
   ...
   `dFluxDiv` という公開演算子の展開に下ろす。幾何のみの係数 local は formurae-post が
...
## 8. 大きなモデルで判明した生成時の落とし穴(2026-09-20)

`examples/kinetic_free_surface` の再構成規則を追加する過程で，DSL の書き方が生成コードの
正しさと大きさを左右する例が三つ見つかった．いずれも処理系の変更ではなく，モデル側の
書き方で回避した．

- 小さい定数リテラル(例 `0.000000000000000001`)は，生成段階で `1/1000000000000000000` が
  整数除算として 0 に畳まれる．分母の下限などは `param` で実行時の値として渡す．
- `a/b` は `a * b^(-1)` として生成されるため，分母が非正規化数まで小さくなると逆数が無限大になり
  `0*inf = NaN` が生じる．分母には正の下限を足す．
- 差の二乗を `def` の引数や式の中に直接書くと，CAS が `(n-s)^2/h^2` を `n^2/h^2 - 2ns/h^2 + s^2/h^2`
  に展開し，ほぼ満杯のセルでは桁落ちで負の値や 0/0 になる．差は `local` や `static field` に
  束ねてから二乗する．
- 隣接 4 セルの被覆率の式を一つの静的場に展開したところ，FMR の 1 行が 9.6 MB になり，
  正規化に 42 分，Formura が 7 分で終わらなくなった．各隣接値を個別の静的場にすると 48 KB に戻る．
- 面ごとの再構成を局所変数のまま流束の発散に使うと，Formura が局所変数を利用箇所ごとに展開して
  C が 30 MB を超え，生成が 45 分でも終わらなかった．`estimatedHeight` と同じく，
  位置決めの段階で状態場に保存してから流束で参照する．
