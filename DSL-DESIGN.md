...
3. **`fec` の特殊規則を減らし、演算子を通常定義へ寄せる**
   ...
   特別扱いを持つ。`Δ`/`Δ4` は `.fe` 側の通常定義へ移したので、この方針を進め、
   `curl`・`divg`・`grad` なども、可能な限り座標文脈つき `use` 定義または
   `.fe`/Egison 側の通常の演算子定義として表現する。
   ...
   - `∂symbol` / `∂m,rsymbol`: 座標軸 `symbol` に沿う偏微分/差分生成子
     (`∂x`, `∂2x`, `∂2,2theta` など)

   `d_symbol` のような綴りは `_` が下添字と衝突し、`∂_i`(添字付き微分演算子)と
   `∂x`(座標軸 x の微分)の区別も読みにくい。非ユークリッド座標・一般計量へ進むなら、
   軸名に対する微分は `∂x`/`∂2x`/`∂2,2x` のように演算子名として書き、
   添字を持つ微分は別途 `∂_i`/`∂~i` などとして扱う方が安全。

   その上で、`grad`・`div`・`curl`・`dForm`・`hodge`・`codiff`・`lb` は
   組み込みではなく `use` で座標文脈から生成される定義、または `.fe` の通常定義へ落とす。
