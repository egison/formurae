...
`∂2x e` を2階中心差分、`∂m,rx e` を m 階微分・半径 r の中心 stencil
として追加した。例えば `∂2,2x e` は `[-2,-1,0,1,2]` の5点 stencil で、
係数は `taylorStencil` が Taylor 条件から導出する。これにより `Δ` と `Δ4`
は組み込みではなく `.fe` 側で
`def Δ u = ∂2x u + ∂2y u + ∂2z u`、
`def Δ4 u = ∂2,2x u + ∂2,2y u + ∂2,2z u` のように書く方針へ移した。
...
弾性波は `v'~i = v~i + (dt/ρ0) * ∂_j σ~i~j`・`σ'~i~j = … λ δ~i~j …` の
...
step:
  ...
  σ'~i~j  = σ~i~j + dt * (la * δ~i~j * ∂_k v'~k
                          + mu * (δ~i~k * ∂_k v'~j
                                + δ~j~k * ∂_k v'~i))
```
...
5. ✅ v1.7(2026-07-10): **数式演算子と Einstein 添字記法** — レビュー指摘
   ...
   `σ'~i~j = σ~i~j + dt * (la * δ~i~j * ∂_k v'~k + mu * (δ~i~k * ∂_k v'~j + δ~j~k * ∂_k v'~i))`。
...
## 6. 次の開発目標: 数式展開を Egison 側へ集約する

現仕様は、Egison のテンソル添字記法・微分形式・CAS を **実装内部の意味論**
として活用している。次の方針は、これを Formurae 側に重複実装しないこと。
Formurae は座標文脈、field layout、staggered 配置、Formura 出力を管理する薄い層に保ち、
数式の展開は Egison 側の定義に任せる。
...
3. **`.fe` の `def` は scalar/stencil 定義に限定する**
   `def Δ u = ∂2x u + ∂2y u + ∂2z u` や
   `def Δ4 u = ∂2,2x u + ∂2,2y u + ∂2,2z u` のような表層置換は `.fe` に残す。
   一方で `def curl X~i = ...` や `def grad u_i = ...` のような rank-changing
   添字演算子は `.fe` の通常 `def` としては扱わない。必要な数式抽象は
   Egison 側の定義として生成するか、将来 Egison ライブラリとして切り出す。
