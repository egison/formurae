...
既存 metric 5例の `.fmr` は移行前と一致する。現在は1モデルにつき1個の unindexed
collocated scalar source に限定する。複数 request、metric-aware form Hodge、
`flat` / `sharp` は未実装である。
...
座標依存の `feFormDerivative` だけを渡す。`FMR.formEqs` は whole-form equation
から Formura storage を生成するため、生成コードの `formComps` / `formDeg` と
成分別 form RHS は削除した。Phase 4 の共有 tensor operator
`FE.grad` / `FE.dGrad` / `FE.divg` / `FE.curl` / `FE.hessian` も実装済みで、
generated equation path への統合が次の作業である。

**v1.32(2026-07-11): Egison 中心の tensor equation への移行開始** —
テンソル・添字・格子配置の意味論を `fec` から Egison へ移す設計を
`design/20260711-egison-centered-grid-semantics.md` に定めた。配置は将来
...
Formura runtime には target tensor と tensor-valued RHS を受け取る `FMR.tensorEqs`
を追加し、collocated indexed vector の生成を成分別 `feqE1...` から単一の
tensor RHS へ変更した。target leaf の `E'_1` 等が Formura storage LHS の情報源に
なるため、数式の成分値と出力名リストを別々に生成しない。Phase 2/3ではHaskell側の
field descriptorとindex-parity placementを3 policyへ統一した。実装状況は
上の v1.33 と設計文書の checklist を参照する。
...
として活用し始めている。`fec` はまだ def/prelude 展開、添字・縮約検査、未移行演算子の
成分特殊化、backend planning を担当する。一方、GridPolicy parity、whole-tensor/form printer、
微分形式、純粋な metric/Laplace--Beltrami 公式は Egison library へ移行済みであり、
最終的には座標文脈・field layout・storage planning と診断へ責務を縮める。
...
4. **計量と Laplace--Beltrami の純粋な公式は Egison にある**
   ...
   `LbPlan` / `AuxFieldPlan` が coefficient / volume / flux field を materialize する。

5. **添字 equation は strict に保つ**
...
6. **残課題**
   ...
   Egison 本体の添字規則との差分を小さくするテスト群を追加する。加えて Phase 4 の
   generated equation path と共有 tensor operator の統合、複数 `lb` request の
   scheduling、metric-aware form Hodge、`flat` / `sharp` が残る。
