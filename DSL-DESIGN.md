...
**v1.26(2026-07-09): 添字つきユーザ定義テンソル演算子** —
`def grad u_i = ∂_i u`、`def div X = ∂_i X~i`、
`def lap u = g~i~j * ∂_i ∂_j u` のような `.fe` 定義を受ける。
結果添字を持つ `def` は呼び出し側でも `grad u_i` のように結果添字を明示し、
その添字名を定義本体へ代入してから既存の strict Einstein 展開に渡す。
結果添字を持たない `def` でも本文に添字縮約を書けるため、
...
metric を明示する。`elastic3d.fe` は `metric g` と対称反変応力
...
strict Einstein 検査を行い、field layout から独立成分を列挙して成分ごとに
...
## 6. 次の開発目標: 添字定義を Egison の記述力へ寄せる

現仕様は、Egison のテンソル添字記法・微分形式・CAS を **実装内部の意味論**
として活用している。次の方針は、`.fe` でも同じ抽象度の定義を書けるようにしつつ、
Formurae は座標文脈、field layout、staggered 配置、Formura 出力を管理する薄い層に保つこと。
第一段階として、単一引数の添字つき `def` を strict Einstein 展開へ接続した。

1. **添字補完は Formurae に実装しない**
   Egison には tensor 値に添字情報を保持する仕組みと、微分形式の omitted-index
   completion がある。Formurae で同じ一般規則を再実装すると、意味論が二重化する。
   そのため `E'~i = E~i + dt * curl B` のように添字付き式へ添字なし vector 関数を
   混ぜる形は許さず、ベクトル解析風に書くなら `E' = E + dt * curl B` と
   添字なし vector equation として書く。

2. **`use` は生成 Egison 定義を選ぶ宣言にする**
   `use vector-calculus { curl, divg }` は、Formurae が `curl` や `divg` を
   Haskell 側で展開する合図ではない。現在の `dimension`/`axes` から
   `dGrad`、`curl`、`divg` の Egison 定義を生成 `.egi` に出す合図である。
   `fec` は `curl B` を `curl B_#` のような Egison の vector 値呼び出しへつなぎ、
   vector equation の component lowering で `(curl B_#)_i` を取り出すだけにする。

3. **`.fe` の `def` は添字定義も受ける**
   `def Δ4 u = ∂ 2 2 x u + ∂ 2 2 y u + ∂ 2 2 z u` のような scalar/stencil 定義に加え、
   `def grad u_i = ∂_i u` のような結果添字つき定義を受ける。
   `def div X = ∂_i X~i` や `def lap u = g~i~j * ∂_i ∂_j u` のように、
   scalar 返りの定義本体にも Einstein 添字縮約を書ける。
   ただし現段階では、呼び出し側に必要な結果添字を明示する単一引数定義に限る。

4. **添字 equation は strict に保つ**
   `v'~i = v~i + (dt/ρ0) * ∂_j σ~i~j` のような添字 equation では、
   各項の自由添字が LHS と上下まで一致し、ダミー添字は上1・下1だけを許す。
   上げ下げは自動化せず、必要なら `metric g` で宣言した計量を明示する。

この方向では、Formurae の新規性は「Egison の数式記法と CAS を、座標文脈つきの
分散ステンシルコード生成へ接続する薄い表層言語」に集中する。
